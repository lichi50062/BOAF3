...
// 95.12.20 調整 A01 SQL by 2295
// 96.04.30 調整 存放比率若為負數,以0顯示 by 2295
// 98.08.25 add 寫入OPERATION_LOG by 2295
// 98.10.13 add 存款平均利率/放款平均利率 by 2295
// 99.05.10 調整 套用DAO.preparestatment,並列印轉換後的SQL by 2295
...
//106.03.17 add 29.資產field_ASSETS 農會A01.190000,漁會A01.100000 by 2295
//          add 30.負債field_DEBT 農會A01.210000+A01.220000+A01.240000+A01.250000+A01.260000,漁會A01.200000 by 2295
//106.12.06 add 已裁撤機構.不產生統計資料 by 2295
//106.12.25 add 淨值佔風險性資產比率總計.是以SUM(910400)/SUM(910500)去計算.不是以91060P/單位數 by 2295       
//106.12.28 add 31.建築放款field_992710(A99.992710) by 2295
//108.09.09 add 32.存放比率-存款總餘額 fieldi_y by 2295
//111.06.23 add 單位數count_seq by 2295
%>
...
sqlCmd.append("       field_ASSETS,field_DEBT, ");//106.03.17 add 資產field_ASSETS/負債field_DEBT
sqlCmd.append("       field_DEBIT,field_CREDIT, ");
sqlCmd.append("       decode(sign(field_DC_RATE - 0),-1,0,field_DC_RATE) as field_DC_RATE,");//96.04.30 調整 存放比率若為負數,以0顯示      
...
sqlCmd.append("       field_992710, ");//--106.12.28 建築放款(A99.992710)
sqlCmd.append("       fieldI_Y ");//--108.09.09 存放比率-存款總餘額
sqlCmd.append(" from (");
...
sqlCmd.append("      		 field_SEQ, ");
sqlCmd.append("      		 round(field_ASSETS /1,0)  as field_ASSETS, ");//106.03.17 add
sqlCmd.append("      		 round(field_DEBT /1,0)  as field_DEBT, ");//106.03.17 add
...
//sqlCmd.append("      		 round(field_CAPTIAL / COUNT_SEQ /  1000  ,2)  as   field_CAPTIAL_RATE, ");//106.12.25 調整
sqlCmd.append("      		 round(field_CAPTIAL / 1000  ,2)  as   field_CAPTIAL_RATE, ");//106.12.25 add
...
sqlCmd.append("              round(field_992710 /1,0)  as field_992710, ");// --106.12.28 add 
sqlCmd.append("              round(a01.fieldI_Y /1,0)  as fieldI_Y ");// --108.09.09 add 存放比率-存款總餘額
...
//sqlCmd.append("                  SUM(a05.field_CAPTIAL)  as  field_CAPTIAL,");//106.12.25 調整  
sqlCmd.append(" 			     decode(SUM(field_910400),0,0,round(SUM(field_910400) * 100000 /SUM(field_910500),0)) as field_CAPTIAL, ");//106.12.25 add
...
sqlCmd.append("                  SUM(field_ASSETS)        field_ASSETS, ");// --106.03.17 add
sqlCmd.append("                  SUM(field_CREDIT)        field_DEBT, ");  // --106.03.17 add
sqlCmd.append("                  SUM(a99.field_992710)    field_992710 "); // --106.12.28 add
...
sqlCmd.append("					           round(sum(decode(bank_type,'6',decode(a01.acc_code,'190000',amt,0),'7',decode(a01.acc_code,'100000',amt,0),0)) /1,0) as field_ASSETS,");//--資產 106.03.17 add
sqlCmd.append("					           round(sum(decode(bank_type,'6',decode(a01.acc_code,'210000',amt,'220000',amt,'240000',amt,'250000',amt,'260000',amt,0),'7',decode(a01.acc_code,'200000',amt,0),0)) /1,0) as field_DEBT,");//--負債 106.03.17 add
...
sqlCmd.append("                    left join (select * from bn01 where m_year=? and bn_type !='2')bn01 on wlx01.bank_no=bn01.bank_no  and bn01.bank_type in('6','7') ");
...
sqlCmd.append("                 ) a01,(select bank_code,");//sum(amt) as  field_CAPTIAL  ");
//106.12.25 調整 以SUM(910400)/SUM(910500)去計算.不是以91060P/單位數 by 2295
sqlCmd.append("                        round(sum(decode(a05.acc_code,'910400',amt,0)) /1,0) as field_910400,");//106.12.25 add
sqlCmd.append("                        round(sum(decode(a05.acc_code,'910500',amt,0)) /1,0) as field_910500 ");//106.12.25 add 
sqlCmd.append("                        from a05 where a05.m_year= ? and a05.m_month = ? and  a05.ACC_code in ('910400','910500','91060P')");
...
sqlCmd.append("                      ,(select bank_code,SUM(decode(a99.acc_code,'992710',a99.amt,0)) as field_992710");// --106.12.28 add
sqlCmd.append("                        from a99 where a99.m_year  = ? and a99.m_month  = ? and a99.ACC_code in ('992710')");
sqlCmd.append("                        group by bank_code) a99 ");
paramList.add(s_year);
paramList.add(s_month);
paramList.add(s_year);
paramList.add(s_month);
paramList.add(s_year);
paramList.add(s_month);
paramList.add(s_year);//106.12.28 add
paramList.add(s_month);//106.12.28 add
sqlCmd.append("			where a01.bank_no=a05.bank_code(+) and a01.bank_no=a04.bank_code(+) and a01.bank_no=a02.bank_code(+) and a01.bank_no=a99.bank_code(+) and a01.bank_no <> ' ' ");
...
sqlCmd.append("       select a01.hsien_id ,  a01.hsien_name,  a01.FR001W_output_order,a01.bank_type,a01.bank_no , a01.bank_name, COUNT_SEQ, ");
sqlCmd.append("      		 field_SEQ, ");
sqlCmd.append("      		 round(field_ASSETS /1,0) as field_ASSETS, ");//--資產106.03.17 add
sqlCmd.append("      		 round(field_DEBT /1,0)   as field_DEBT, ");//--負債106.03.17 add
sqlCmd.append("      		 round(field_DEBIT /1,0)  as field_DEBIT, ");
sqlCmd.append("      		 round(field_CREDIT /1,0) as field_CREDIT, ");
sqlCmd.append("         	 decode(a01.fieldI_Y,0,0, ");
sqlCmd.append("          	 round( ");
...
sqlCmd.append("                  SUM(a02.field_990611) as field_990611,");// --102.07.03 add   
sqlCmd.append("                  SUM(field_ASSETS)        field_ASSETS,");// --106.03.17 add
sqlCmd.append("                  SUM(field_DEBT)          field_DEBT,");// --106.03.17 add  
sqlCmd.append("                  SUM(a99.field_992710)    field_992710 "); // --106.12.28 add
sqlCmd.append(" 			from (  select nvl(cd01.hsien_id,' ')       as  hsien_id , ");
sqlCmd.append("                            nvl(cd01.hsien_name,'OTHER')  as  hsien_name, ");
sqlCmd.append("                            cd01.FR001W_output_order     as  FR001W_output_order, ");
...
sqlCmd.append("					           round(sum(decode(a01.acc_code,'120000',amt,'120800',amt,'150300',amt,0)) /1,0) as  field_CREDIT,    ");                           
sqlCmd.append("							   round(sum(decode(bank_type,'6',decode(a01.acc_code,'190000',amt,0),'7',decode(a01.acc_code,'100000',amt,0),0)) /1,0) as field_ASSETS,");//--資產 106.03.17 add
sqlCmd.append("							   round(sum(decode(bank_type,'6',decode(a01.acc_code,'210000',amt,'220000',amt,'240000',amt,'250000',amt,'260000',amt,0),'7',decode(a01.acc_code,'200000',amt,0),0)) /1,0) as field_DEBT,");//--負債106.03.17 add
sqlCmd.append("	           		           round(sum(decode(YEAR_TYPE,'102',decode(bank_type,'6',decode(a01.acc_code,'120101',amt,'120102',amt,  ");
sqlCmd.append("	            	                                                                         '120200',amt,'120301',amt,'120302',amt,'120700',amt,'150200',amt,0)  "); 
sqlCmd.append("	            	                                                            ,'7',decode(a01.acc_code,'120101',amt,'120102',amt,  ");
sqlCmd.append("	            	                                                                         '120300',amt,'120401',amt,'120402',amt,'120700',amt,'150200',amt,0)), ");
...
sqlCmd.append("                         a01.field_ASSETS,a01.field_DEBT,");// --106.03.17 add
sqlCmd.append("                         a99.field_992710");//106.12.28 add
sqlCmd.append(" 				from ( select nvl(cd01.hsien_id,' ')       as  hsien_id , ");
sqlCmd.append("        					      nvl(cd01.hsien_name,'OTHER')  as  hsien_name, ");
...
sqlCmd.append("                               round(sum(decode(a01.acc_code,'220000',amt,0)) /1,0)        as field_DEBIT,  "); 
sqlCmd.append("					              round(sum(decode(a01.acc_code,'120000',amt,'120800',amt,'150300',amt,0)) /1,0) as  field_CREDIT,    ");   
sqlCmd.append("					              round(sum(decode(bank_type,'6',decode(a01.acc_code,'190000',amt,0),'7',decode(a01.acc_code,'100000',amt,0),0)) /1,0) as field_ASSETS,");//--資產106.03.17 add
sqlCmd.append("					              round(sum(decode(bank_type,'6',decode(a01.acc_code,'210000',amt,'220000',amt,'240000',amt,'250000',amt,'260000',amt,0),'7',decode(a01.acc_code,'200000',amt,0),0)) /1,0) as field_DEBT,");//--負債106.03.17 add
sqlCmd.append("	           		              round(sum(decode(YEAR_TYPE,'102',decode(bank_type,'6',decode(a01.acc_code,'120101',amt,'120102',amt,  ");
sqlCmd.append("	            	                                                                            '120200',amt,'120301',amt,'120302',amt,'120700',amt,'150200',amt,0)  "); 
sqlCmd.append("	            	                                                               ,'7',decode(a01.acc_code,'120101',amt,'120102',amt,  ");
...
sqlCmd.append("					          round(sum(decode(a01.acc_code,'120000',amt,'120800',amt,'150300',amt,0)) /1,0) as  field_CREDIT,    ");    
sqlCmd.append("					          round(sum(decode(bank_type,'6',decode(a01.acc_code,'190000',amt,0),'7',decode(a01.acc_code,'100000',amt,0),0)) /1,0) as field_ASSETS,");//--資產106.03.17 add
sqlCmd.append("					          round(sum(decode(bank_type,'6',decode(a01.acc_code,'210000',amt,'220000',amt,'240000',amt,'250000',amt,'260000',amt,0),'7',decode(a01.acc_code,'200000',amt,0),0)) /1,0) as field_DEBT,");//--負債106.03.17
sqlCmd.append("	           		          round(sum(decode(YEAR_TYPE,'102',decode(bank_type,'6',decode(a01.acc_code,'120101',amt,'120102',amt,  ");
sqlCmd.append("	            	                                                                        '120200',amt,'120301',amt,'120302',amt,'120700',amt,'150200',amt,0)  "); 
sqlCmd.append("	            	                                                           ,'7',decode(a01.acc_code,'120101',amt,'120102',amt,  ");
sqlCmd.append("	            	                                                                        '120300',amt,'120401',amt,'120402',amt,'120700',amt,'150200',amt,0)), ");
...
sqlCmd.append("	)  a01  ORDER by    FR001W_output_order, field_SEQ,  hsien_id ,  bank_no ");      
    ...
    //28.放款覆蓋率(備抵呆帳/放款總額)field_BACKUP_CREDIT_RATE 29.資產field_ASSETS 30.負債 field_DEBT 31.建築放款 field_992710 32.存放比率-存款總餘額fieldI_Y
