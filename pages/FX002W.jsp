...
//106.08.02 fix 分支機構代號無法顯示問題(編輯時增加傳入tbank_no/bank_no) by 2295
//106.10.13 add 原機構代碼、轉換日期 by 2295
//106.11.23 add 機構代碼轉換日期;原轉換日期調整為MIS系統轉換日期 by 2295
//111.03.02調整寫入WLX02_LOG欄位不足問題及回查詢頁 by 2295
%>
...
<%
    ...
    if(!Utility.CheckPermission(request,"FX002W")){//無權限時,導向到LoginError.jsp 
    	...
    	if(act.equals("List") || act.equals("Query")){ //for list and Query   	    
			...
			rd = application.getRequestDispatcher( EditWLX02PgName +"?tbank_no="+tbank_no+"&bank_no="+bank_no );//106.08.02 fix 加入tbank_no,bank_no        
        	...
        	rd = application.getRequestDispatcher(nextPgName+"?FX=FX002W");        		
        	...
        	rd = application.getRequestDispatcher( nextPgName+"?FX=FX002W");//111.03.02調整回查詢頁            		
...
<%!
    ...
    private List getWLX02(String tbank_no,String bank_no){
    		...
    		List paramList = new ArrayList() ;
    		String yy = Integer.parseInt(Utility.getYear())>99 ?"100":"99" ;
    		sqlCmd.append(" select WLX02.*,bank_name,exchange_no, "); 
        	...
        	sqlCmd.append(" from (select bn02.*,online_date from bn02 left join ba01_trans on bn02.ori_bank_no= ba01_trans.src_bank_no where m_year=?)bn02 ");
        	sqlCmd.append(" LEFT JOIN (select * from wlx02 where m_year=?)wlx02 on wlx02.tbank_no = bn02.tbank_no and wlx02.bank_no=bn02.bank_no ");
        	sqlCmd.append(" where bn02.tbank_no=? and bn02.bank_no=? "); 
    		paramList.add(yy) ;
    		paramList.add(yy) ;
    		paramList.add(tbank_no)  ;
    		paramList.add(bank_no) ;
            List dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),paramList,"setup_date,setup_no_date,chg_license_date,start_date,staff_num,open_date,cancel_date,update_date,exchange_no,trans_date,online_date,refer_date");            
            return dbData;
    }
	...
	private String UpdateWLX02(HttpServletRequest request,String tbank_no,String bank_no,String lguser_id,String lguser_name) throws Exception{    	
	    ...
	    String refer_date = (request.getParameter("REFER_DATE")==null)?"":(String)request.getParameter("REFER_DATE");
	    String refer_no = (request.getParameter("REFER_NO")==null)?"":(String)request.getParameter("REFER_NO");
		...
		try {
				...
				if (data.size() == 0){//無資料時,Insert
					sqlCmd.setLength(0) ;
				    paramList.clear() ;
					sqlCmd.append("INSERT INTO WLX02 (");
					sqlCmd.append(" REFER_DATE,REFER_NO,TBANK_NO,BANK_NO,CONST_TYPE,SETUP_APPROVAL_UNT,SETUP_DATE,SETUP_NO,");
					sqlCmd.append(" SETUP_NO_DATE,CHG_LICENSE_DATE,CHG_LICENSE_NO,CHG_LICENSE_REASON,");
					sqlCmd.append(" START_DATE,HSIEN_ID,AREA_ID,ADDR,TELNO,FAX,EMAIL,WEB_SITE,FLAG,");
					sqlCmd.append(" OPEN_DATE,STAFF_NUM,HSIEN_DIV_1,CANCEL_NO,CANCEL_DATE,");
					sqlCmd.append(" USER_ID,USER_NAME,UPDATE_DATE,M_YEAR,BUSINESS_ITEM,BUSINESS_ITEM_EXTRA,EXTRA_INFO");
					...
					paramList.add(refer_date) ;
				    paramList.add(refer_no);
					paramList.add(tbank_no) ;
				    paramList.add(bank_no);
					...
					paramList.add(business_item);//106.03.07 add
				    paramList.add(business_item_extra);//106.3.07 add
				    paramList.add(extra_info);//106.03.07 add
					if(this.updDbUsesPreparedStatement(sqlCmd.toString(),paramList)){	   				      		 
					 ...
					 sqlCmd.setLength(0) ;
				     paramList.clear() ;
				     sqlCmd.append(" UPDATE bn02 SET EXCHANGE_NO=?,update_date=sysdate " 
                    ...
                    sqlCmd.append(" INSERT INTO WLX02_LOG "
				           ...
				           + "		  START_DATE,REFER_DATE,REFER_NO,HSIEN_ID,AREA_ID,ADDR,TELNO,FAX,EMAIL,WEB_SITE,FLAG,OPEN_DATE,STAFF_NUM,"
				           + "		  BUSINESS_ITEM,BUSINESS_ITEM_EXTRA,EXTRA_INFO,HSIEN_DIV_1,CANCEL_NO,CANCEL_DATE,"
						   + "		  USER_ID,USER_NAME,UPDATE_DATE,M_YEAR,"
						   + "        ?,?,sysdate,'U'"
						   + " from WLX02"
				    ...
				    sqlCmd.append(" refer_date=to_date(?,'YYYY/MM/DD')");paramList.add(refer_date) ;
				    sqlCmd.append(",refer_no=?");paramList.add(refer_no) ;
    ...
    //111.03.02調整寫入WLX02_LOG欄位不足問題 by 2295
