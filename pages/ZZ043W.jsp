...
//106.12.28 add 線上產生Axx_operation_month關帳資料 by 2295
...
<%
    ...
    if(!Utility.CheckPermission(request,report_no)){//無權限時,導向到LoginError.jsp
    	...
    	if(act.equals("Qry")){                    	        	    
            ...
            out.print(actMsg);    	    
    	    rd = application.getRequestDispatcher( nextPgName +"?goPages=ZZ043W.jsp&act=Qry&firstStatus="+firstStatus);//111.02.18調整回查詢頁
    	}else if(act.equals("GenClose")){
    	    GenOperationMonth b = new GenOperationMonth();
            b.exeGenOperationMonth(szReport_no,S_YEAR,S_MONTH,"true",lguser_id);
            List paramList = new ArrayList();
            paramList.add(S_YEAR);
            paramList.add(S_MONTH);
            List dbData = DBManager.QueryDB_SQLParam(" select count(*) as datacount from "+szReport_no+"_operation_month where m_year= ? and m_month= ? ",paramList,"datacount");
            if(dbData != null && dbData.size() > 0){
               if(Integer.parseInt(((DataObject)dbData.get(0)).getValue("datacount").toString()) > 0){
                  actMsg = szReport_no+"彈性報表關帳資料已產生完成!!";                  
               }
            }else{
               actMsg =  szReport_no+"彈性報表關帳資料產生失敗!!";
            }
            out.print(actMsg);
    	    rd = application.getRequestDispatcher( nextPgName +"?goPages=ZZ043W.jsp&act=Qry&firstStatus="+firstStatus);//111.02.18調整回查詢頁
    	}	     	
