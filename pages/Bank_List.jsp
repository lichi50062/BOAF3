<%
//94.12.02 調整 農漁會地方主管第二層list by lilic0c0 4183
//93.12.22 調整 農會/漁會無已裁撤之相關機構
//94.01.12 調整 若登入者為農漁會共用中心,則農會.漁會只顯示參加該共用中心的機構代碼 by 2295
//94.03.30 調整 農會/漁會顯示已裁撤之相關機構 by 2295
//         調整 修正FX004W url by 2295
//94.04.12 調整 按bank_no sort by 2295
//95.08.18 add 警示帳戶.地方主管機關進來.可看到其所屬的農漁會.農漁會進來只能看到自己的 by 2295
//95.08.24 add 承受擔保品.警示帳戶.地方主管機關進來.可看到其所屬的農漁會.並出現列印按鈕 by 2495
//99.09.23 調整 根據查詢年度.100年以後取得新機構名稱.100年以前取得舊機構名稱
...
//106.09.30 add 轉換日期 by 2295
//106.11.23 調整 調整顯示機構代號轉換日期 by 2295
//107.08.17 調整 使用bean讀取DataObject by 2295
//108.06.17 add FX008AW/FX009W,報表格式挑選 by 2295		
%>
...
<%
  ...
  //=====================================================================
   ...
   //94.01.12 調整 農會.漁會只顯示參加該共用中心的機構代碼============================
   //94.01.14 調整 農會.漁會只顯示參加該地方主管機關的機構代碼============================
   sqlCmd.append("select bn01.bank_no,bn01.bank_name,bn01.update_date,wlx01.cancel_date,ori_bank_no,online_date from (select * from bn01 where m_year=?)bn01 ");
   sqlCmd.append("left join (select * from wlx01 where m_year=?)wlx01 on bn01.bank_no = WLX01.bank_no ");      
   sqlCmd.append("left join ba01_trans on bn01.ori_bank_no= ba01_trans.src_bank_no ");
   ...
   //94.12.02 調整 農會.漁會只顯示參加該地方主管機關的機構代碼============================
   ...
   //=================================================================================
   List dbData_online = DBManager.QueryDB_SQLParam(sqlCmd_normal.toString(),paramList_normal,"online_date");		  
   //sqlCmd = "select bank_no,bank_name,update_date from bn01 where bank_type='"+bank_type+"' and bn_type = '2'";
   
   List dbData_revoke = DBManager.QueryDB_SQLParam(sqlCmd_revoke.toString(),paramList_revoke,"update_date,cancel_date,online_date");		  
                ...
                %>
                    ...
                    <%}%>
                          ...
                          <tr class="sbody" bgcolor='#D8EFEE'>
						  	...
						  	<td width='40%' align='left'>機構名稱</td>
						  	<td width='30%' align='left'>轉換日期</td>
	  					  ...
	  					  <%
	  					    ...
	  					    if(dbData_online != null && dbData_online.size() != 0){	  					       	
	  					       while(i<dbData_online.size()){
	  					      	 ...
	  					      	 bean = (DataObject)dbData_online.get(i);//107.08.17 add
	  					  %>
						  <tr class="sbody" bgcolor='<%=bgcolor%>'>
						  	<td width='30%'  align='left'>
						  	<% if(!list_type.equals("")){%>
						      <%if(program_url.startsWith("FX004W")){%>
						       <a href='<%=program_url%>&bank_no=<%=(String)bean.getValue("bank_no")%>'>
						      <%}else{%>
						       <a href='<%=program_url%>&tbank_no=<%=(String)bean.getValue("bank_no")%>&bank_type=<%=bank_type%>&list_type=<%=list_type%>'>                          
						      <%}%>
                          <%}else{%>
                              <%if(program_url.startsWith("FX004W")){%>
						        <a href='<%=program_url%>&bank_no=<%=(String)bean.getValue("bank_no")%>'>
						      <%}else{%>
                                <a href='<%=program_url%>&tbank_no=<%=(String)bean.getValue("bank_no")%>&bank_type=<%=bank_type%>'>
                              <%}%>  
                          <%}%>
						  <%=(String)bean.getValue("bank_no")%>
						  </a>
						  </td>
						  <td width='40%'>
						  <%if(!list_type.equals("")){%>
						  ...
						  <%}else{%>
						        ...
						        <%}%>   
						  <%}%>
						  <%if(bean.getValue("online_date") != null && !(bean.getValue("online_date")).equals("")){%>
						  <%=Utility.getCHTdate((bean.getValue("online_date")).toString().substring(0, 10), 0)%>
						  <%}else{%>
						  	&nbsp;
						  <%}%>	
						  </a>
						  ...
						  <td width='30%' align='left' >裁撤日期</td>
	  					  ...
	  					  <%i = 0;
	  					    if(dbData_revoke != null && dbData_revoke.size() != 0){
	  					       ...
	  					       bgcolor = (i % 2 == 0)?"#e7e7e7":"#D3EBE0";		
	  					       bean = (DataObject)dbData_revoke.get(i);
	  					  %>
						  <tr class="sbody" bgcolor='<%=bgcolor%>'>
						  <td width='30%' align='left'>
						  <%if(program_url.startsWith("FX004W")){%>
						  <a href='<%=program_url%>&bank_no=<%=(String)bean.getValue("bank_no")%>'>
						  <%}else{%>
						  <a href='<%=program_url%>&tbank_no=<%=(String)bean.getValue("bank_no")%>'>
						  <%}%>
						  <%=(String)bean.getValue("bank_no")%>
						  </a>
						  </td>
						  ...
						  <a href='<%=program_url%>&tbank_no=<%=(String)bean.getValue("bank_no")%>'>
						  <%}%>
						  <%=(String)bean.getValue("bank_name")%>
						  </a>
						  </td>
						  <td width='40%' >
						  <%if(bean.getValue("cancel_date") != null && !(bean.getValue("cancel_date")).equals("")){%>
						  <a href='<%=program_url%>&tbank_no=<%=(String)bean.getValue("bank_no")%>'>
						  <%if(bean.getValue("cancel_date") != null && !(bean.getValue("cancel_date")).equals("")){
						        out.print(Utility.getCHTdate((bean.getValue("cancel_date")).toString().substring(0, 10), 0));						  
						    }    
						  %>
						  </a>
						  <%}else{
