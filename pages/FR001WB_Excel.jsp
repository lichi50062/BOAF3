...
<%
   ...
   String printStyle = Utility.getTrimString(dataMap.get("printStyle")); //108.03.12 add
    ...
    if(!Utility.CheckPermission(request,report_no)){//無權限時,導向到LoginError.jsp
    	rd = application.getRequestDispatcher( LoginErrorPgName );
	  	...
	  	if(act.equals("Qry")){
    	   ...
    	   //就是靠這一行，讓前端瀏覽器以為接收到一個excel檔
    		response.setHeader("Content-disposition","inline; filename=view."+printStyle);//108.03.12調整顯示的副檔名
    	}else if (act.equals("download")){
    		response.setHeader("Content-Disposition","attachment; filename=download."+printStyle);//108.03.12調整顯示的副檔名
    	}
    	if(act.equals("view") || act.equals("download")){
    		try{
	  			...
	  			if(!printStyle.equalsIgnoreCase("xls")) {//108.03.12非xls檔須執行轉換	                
