<%
// 95.8.21 created by lilic0c0 2495
...
<%
	...
	String printStyle = ( request.getParameter("printStyle")==null ) ? "" : (String)request.getParameter("printStyle");//108.06.17
	response.setContentType("application/msexcel;charset=UTF-8");//以上這行設定本網頁為excel格式的網頁
	response.setHeader("Content-Disposition","attachment; filename=download."+printStyle);//108.06.17調整顯示的副檔名      
	...
	if(act.equals("view")){
		...
		response.setHeader("Content-disposition","inline; filename=view."+printStyle);//108.06.17調整顯示的副檔名      
...
<%
	try{
	    ...
	    if(!printStyle.equalsIgnoreCase("xls")) {//108.06.17非xls檔須執行轉換	                
