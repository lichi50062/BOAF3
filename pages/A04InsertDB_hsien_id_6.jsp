...
// 99.11.05 調整 套用DAO.preparestatment,並列印轉換後的SQL by 2295
//103.08.18 調整 field_RATE_840760_OVER_A04欄位太長,造成sql error(改成field_840760_OVER_A04) by 2295
//111.07.12 調整增加g.v_bank_location.bank_type設定 by 2295
%>
...
sqlCmd.append("   group by nvl(e.hsien_id,' '),nvl(e.hsien_name,'其他'),e.fr001w_output_order) f,(select * from v_bank_location where m_year=? and bank_type=?)g  ");
...
paramList.add(bank_type);
paramList.add(wlx01_m_year);
paramList.add(bank_type);//111.07.12 add
    ...
    for(int i=0;i<dbData.size();i++){        
	    ...
	    for(int j=0;j<field.length;j++){	       
	    	System.out.println("field[j]="+field[j]);
	    	System.out.println("hsien_id="+(String)bean.getValue("hsien_id"));
	    	System.out.println("field_type[j]="+field_type[j]);
