...
//111.03.21 調整Edge[更新案件編號]/[新增下一筆參貸項目].無反應 by 2295
//111.09.06 fix 分項為null時,無法繳輯原新增資料 by 2295
%>
...
<!-- -----------------------------end of hide type---------------------------- -->
        ...
        <tr>
			...
			<td width='398' bgcolor='e7e7e7' height="10">			    
               	...
               	<input type="button" value="更新案件編號" onclick="javascript:updateCase_No(document.UpdateForm,'<%=hyear%>','<%=hmonth%>','<%=bank_no%>','<%=seq_no%>');">               	
            </td>  	
        </tr>			
	    <%}%>
        <tr>
			...
			<td width='398' bgcolor='e7e7e7' height="10">			    
    		     ...
    		     <% if(A11_S_Edit != null && A11_S_Edit.size() != 0){ 
	    		     	if(Bean.getValue("case_no") != null && Bean.getValue("case_cnt") != null && (case_no).equals(maxCase_No) && (Integer.parseInt(maxCase_No.substring(7, 9))) != Integer.parseInt(case_cnt) ){ %>
	    		     		<input type="button" value="新增下一筆參貸項目" onclick="javascript:continueEditCnt(document.UpdateForm,'<%=hyear%>','<%=hmonth%>','<%=bank_no%>','<%=case_no%>');">
						<%}
    		     	}%>
			</td>
        </tr>
