...
<%
	...
	//106.02.20 add A10欄位 by 2968
	//107.02.07 fix A10上月資料匯入失敗問題 by 2295
	...
	//110.08.16 add A99.amt_name by 2295[111.06.13取消]
	//111.03.18 fix Insert into A02_LOG add amt_name1,amt_name2 by 2295
	//111.03.18 fix Insert into A99_LOG add a99.amt_name by 2295[111.06.13取消]
	//111.08.24 add A02增加勾選991311/991321的農金局回文函號 by 2295
%>
...
//add by egg 2004.12.21
<%@ page import="com.tradevan.util.UpdateM08"%>
//add by jei 2004.12.21
<%@ page import="com.tradevan.util.UpdateF01"%>
//add by 2295 2005.11.10
<%@ page import="com.tradevan.util.UpdateA99"%>
//add by 2295 2006.05.25
<%@ page import="com.tradevan.util.UpdateA08"%>
//add by 2295 2007.07.10
<%@ page import="com.tradevan.util.UpdateA09"%>
//add by 2295 2007.12.03
<%@ page import="com.tradevan.util.UpdateA10"%>
//add by 2295 2008.06.13
<%@ page import="com.tradevan.util.UpdateA12"%>
//add by 2968 2015.01.13
<%@ page import="com.tradevan.util.UpdateA13"%>
//add by 2295 2015.10.12
<%@ page import="com.tradevan.util.UpdateA15"%>
//add by 6493 2020.07.08
...
<%
	...
	String username = (session.getAttribute("muser_name") == null) ? ""
			: (String) session.getAttribute("muser_name");
	//======================================================================================================================
	//fix 93.12.18 若有已點選的tbank_no,則以已點選的tbank_no為主============================================================
	String bank_code = (session.getAttribute("tbank_no") == null) ? ""
			: (String) session.getAttribute("tbank_no");
	...
	bank_code = (session.getAttribute("nowtbank_no") == null) ? bank_code
			: (String) session.getAttribute("nowtbank_no");
	//=======================================================================================================================
	//fix 93.12.20 若有已點選的bank_type,則以已點選的bank_type為主============================================================
	String bank_type = Utility.getTrimString(dataMap.get("bank_type"));
	bank_type = (session.getAttribute("nowbank_type") == null) ? bank_type
			: (String) session.getAttribute("nowbank_type");
	//=======================================================================================================================
	if (session.getAttribute("nowbank_type") == null)
		System.out.println("nowbank_type == null");
	else
		System.out.println((String) session.getAttribute("nowbank_type"));
	if (!Utility.CheckPermission(request, report_no)) {//無權限時,導向到LoginError.jsp
		...
		if (act.equals("List")) {
			...
			rd = application.getRequestDispatcher(StatusPgName + "?act=Status&Report_no=" + Report_no
					+ "&bank_code=" + bank_code + "&test=nothing");
		} else if (act.equals("new") || act.equals("getLastMonthData")) {
			if (Report_no.equals("F01") || Report_no.equals("A06") || Report_no.equals("A99") || Report_no.equals("A15")
					|| Report_no.equals("A08") || Report_no.equals("A10")) {//95.05.26 add A99 //96.07.10 add A08 //97.06.13 add A10 //109.07.15 add A15
				...
				System.out.println(
						"WLX_APPLY_INI.size()=" + (((DataObject) dbData.get(0)).getValue("cnt")).toString());
				if (dbData != null && ((DataObject) dbData.get(0)).getValue("cnt") != null) {
					if (Integer.parseInt((((DataObject) dbData.get(0)).getValue("cnt")).toString()) > 0) {
						...
						String[] WLX01_FIRST_APPLY = getWLX01_FIRST_APPLY(Report_no, bank_code); //WML01的初次申報年月
						String[] WLX_APPLY_INI = getWLX_APPLY_INI(Report_no); //WLX_APPLY_INI設定的起始年月
						boolean isUpdate = false, isInitial = false;
						...
						sqlCmd.append(" where aa.BANK_CODE  = ?");
						...
						if(dbData != null && dbData.size() > 0){
							//鍵入的申報年月 與 已申報資料 相同 --> Update
							for(int i=0; i<dbData.size(); i++){
								dataObject = (DataObject)dbData.get(i);
								if(S_YEAR.equals((dataObject.getValue("m_year")).toString()) && String.valueOf(Integer.parseInt(S_MONTH)).equals((dataObject.getValue("m_year")).toString())){
									isUpdate = true;
								}
							}
						}
						...
						if(isUpdate){
							...
							if (Report_no.equals("F01")) {
								...
								if (!checkExistMonth(S_YEAR, S_MONTH, bank_code, "NEXT", Report_no)) {//無下月申報的資料
									rd = application.getRequestDispatcher(EditPgName + Report_no
											+ ".jsp?act=Edit&NextMonthCreated=false&bank_type=" + bank_type
											+ "&S_YEAR=" + S_YEAR + "&S_MONTH="
											+ String.valueOf(Integer.parseInt(S_MONTH)) + "&test=nothing");
								} else {
									rd = application.getRequestDispatcher(EditPgName + Report_no
											+ ".jsp?act=Edit&NextMonthCreated=true&bank_type=" + bank_type
											+ "&S_YEAR=" + S_YEAR + "&S_MONTH="
											+ String.valueOf(Integer.parseInt(S_MONTH)) + "&test=nothing");
								}
								...
								rd = application.getRequestDispatcher(
										EditPgName + Report_no + ".jsp?act=Edit&bank_type=" + bank_type
												+ "&S_YEAR=" + S_YEAR + "&S_MONTH="
												+ String.valueOf(Integer.parseInt(S_MONTH)) + "&test=nothing");
							} else if (Report_no.equals("A15")) {
								boolean isFirstApply=false,isMonth01=false;
								if (S_YEAR.equals(WLX01_FIRST_APPLY[0]) && String.valueOf(Integer.parseInt(S_MONTH)).equals(WLX01_FIRST_APPLY[1])) {
									System.out.println("修改 第一次申報資料");
									request.setAttribute("edit_apply_ini", true);
									isFirstApply = true;
								}
								if(Integer.parseInt(S_MONTH)==1){
									isMonth01 = true;
								}
								data_div01 = getData_A01_A05(S_YEAR, S_MONTH, bank_code, bank_type, "15", isFirstApply, isMonth01, Report_no);
								rd = application.getRequestDispatcher(
										EditPgName + Report_no + ".jsp?act=Edit&bank_type=" + bank_type
												+ "&S_YEAR=" + S_YEAR + "&S_MONTH="
												+ String.valueOf(Integer.parseInt(S_MONTH)) );
								...
								rd = application.getRequestDispatcher(
										EditPgName + Report_no + ".jsp?act=Edit&bank_type=" + bank_type
												+ "&S_YEAR=" + S_YEAR + "&S_MONTH="
												+ String.valueOf(Integer.parseInt(S_MONTH)) + "&test=nothing");
							} else if (Report_no.equals("A08") || Report_no.equals("A09")
									|| Report_no.equals("A10") || Report_no.equals("A12")) {//97.06.13 add A10 
								data_div01 = getData_A08_A12(S_YEAR, S_MONTH, bank_code, Report_no);
								String path = EditPgName + Report_no + ".jsp?act=Edit&bank_type=" + bank_type
										+ "&S_YEAR=" + S_YEAR + "&S_MONTH="
										+ String.valueOf(Integer.parseInt(S_MONTH)) + "&test=nothing";
							...
							nextPg = "true";
							System.out.println("F01/A06/A99/A08/A09/A10/A12/A15 Update");
						} else {
							System.out.println("F01/A06/A99/A08/A09/A10/A12/A15 new");
							//新增資料
							
							//判斷是否為初次申報
							boolean isFirstApply = false;
							if ( WLX01_FIRST_APPLY == null || WLX01_FIRST_APPLY.length<1 || isEmpty(WLX01_FIRST_APPLY[0]) || isEmpty(WLX01_FIRST_APPLY[1]) ) {
								//WLX01無資料則為初次申報
								System.out.println("初次申報");
								isFirstApply = true;
							} else {
								//此次申報年月介於"起始年月"及"WLX01最早年月",為初次申報
								int sYear = Integer.parseInt(S_YEAR);
								int sMonth = Integer.parseInt(S_MONTH);
								int firstApplyYear = Integer.parseInt(WLX01_FIRST_APPLY[0]);
								int firstApplyMonth = Integer.parseInt(WLX01_FIRST_APPLY[1]);
								int InitialYear = Integer.parseInt(WLX_APPLY_INI[0]);
								int InitialMonth = Integer.parseInt(WLX_APPLY_INI[1]);
								
								System.out.println("firstApplyYear = "+firstApplyYear);
								System.out.println("firstApplyMonth = "+firstApplyMonth);
								System.out.println("InitialYear = "+InitialYear);
								System.out.println("InitialMonth = "+InitialMonth);
								
								if( (InitialYear == sYear && InitialMonth <= sMonth || InitialYear < sYear) && 
									(firstApplyYear == sYear && firstApplyMonth > sMonth || firstApplyYear > sYear) ){
									System.out.println("初次申報");
									isFirstApply = true;
								}
							}
							
							System.out.println("isFirstApply = "+isFirstApply);
							System.out.println("isInitial = "+isInitial);
							
							String F01_APPLY_INI = "false";
							if(isFirstApply && isInitial){
								//為初次申報且與農金局設定的起始年月相同
								F01_APPLY_INI = "true";
							}else if(isFirstApply && !isInitial){
								//為初次申報且與農金局設定的起始年月不相同
								alertMsg = "農金局規定本案起始的申報年月為(" + WLX_APPLY_INI[0] + "," + WLX_APPLY_INI[1]
										+ ")" + ",與你第一次將申報年月(" + S_YEAR + "," + S_MONTH + ")不一致" + ",如果你是("
										+ WLX_APPLY_INI[0] + "," + WLX_APPLY_INI[1] + ")之後才新增成立的單位,"
										+ "亦請以成立當月為起始申報年月";
								F01_APPLY_INI = "true";//為新增的總機構單位視同為ini
								webURL_Y = EditPgName + Report_no + ".jsp?act=new&bank_type=" + bank_type
										+ "&S_YEAR=" + S_YEAR + "&S_MONTH="
										+ String.valueOf(Integer.parseInt(S_MONTH)) + "&F01_APPLY_INI="
										+ F01_APPLY_INI + "&test=nothing";
								webURL_N = StatusPgName + "?act=Status&Report_no=" + Report_no
										+ "&bank_code=" + bank_code + "&test=nothing";
								request.setAttribute("actMsg", actMsg);
								...
								if (Report_no.equals("F01")) {
									if (!checkExistMonth(S_YEAR, S_MONTH, bank_code, "LAST", Report_no)) {//無申報成功的資料
										...
										List data_div01 = getLastMonthData(S_YEAR, S_MONTH, bank_code,
												Report_no, bank_type);
										request.setAttribute("data_div01", data_div01);
										//}
									}
									...
									List data_div01 = getLastMonthData(S_YEAR, S_MONTH, bank_code,
											Report_no, bank_type);
									request.setAttribute("data_div01", data_div01);
								} else if (Report_no.equals("A15")) {//109.07.15 上月資料匯入 by 6493 
									List data_div01 = getLastMonthDataAndNewItem(S_YEAR, S_MONTH, bank_code, Report_no, bank_type);
									request.setAttribute("data_div01", data_div01);
									...
									request.setAttribute("data_div01", data_div01);
									rd = application.getRequestDispatcher(EditPgName + Report_no
											+ ".jsp?act=new&bank_type=" + bank_type + "&S_YEAR=" + S_YEAR
											+ "&S_MONTH=" + String.valueOf(Integer.parseInt(S_MONTH))
											+ "&F01_APPLY_INI=" + F01_APPLY_INI
											+ "&test=nothing&width=729");
									nextPg = "true";
									//}
								}
							}
							...
							if (!nextPg.equals("true")) {//沒有設下一頁時,才加預設的next pg by 2295
								rd = application.getRequestDispatcher(EditPgName + Report_no + ".jsp?act=" + act
										+ "&bank_type=" + bank_type + "&S_YEAR=" + S_YEAR + "&S_MONTH="
										+ String.valueOf(Integer.parseInt(S_MONTH)) + "&F01_APPLY_INI="
										+ F01_APPLY_INI + "&test=nothing");
							}
						}
				...
				request.setAttribute("actMsg", actMsg);
				rd = application.getRequestDispatcher(
						EditPgName + Report_no + ".jsp?act=new&bank_type=" + bank_type + "&test=nothing");
			}
			...
			if (Report_no.equals("A01")) {
				...
				request.setAttribute("data_div01", data_div01);
				request.setAttribute("data_990621", Utility.get_ncacno_990621(S_YEAR, S_MONTH));
				...
				request.setAttribute("data_div01", data_div01);
			} else if (Report_no.equals("A08") || Report_no.equals("A09") || Report_no.equals("A10")
					|| Report_no.equals("A12")) {//96.07.10 add by 2295 
				...
				List data_div01 = getData_A01_A05(S_YEAR, S_MONTH, bank_code, bank_type, "15", isFirstApply, isMonth01, Report_no);
				request.setAttribute("data_div01", data_div01);
			...
			request.setAttribute("actMsg", actMsg);
			String path = ".jsp?act=Edit&NextMonthCreated=" + NextMonthCreated + "&bank_type=" + bank_type
					+ "&test=nothing";
			if (Report_no.equals("A10")){
				path += "&width=729";				
			}
			...
			if (!CheckFileLock(S_YEAR, S_MONTH, bank_code, Report_no)) {//94.02.21該檔沒有被LOCK住
				if (Report_no.equals("A01") || Report_no.equals("A02") || Report_no.equals("A03")
						|| Report_no.equals("A04") || Report_no.equals("A05")) {
					actMsg = insertA01_A05(request, String.valueOf(Integer.parseInt(S_YEAR)),
							String.valueOf(Integer.parseInt(S_MONTH)), bank_code, bank_type, Report_no, userid,
							username);
				} else if (Report_no.equals("B01")) {
				...
				} else if (Report_no.equals("F01") || Report_no.equals("A06") || Report_no.equals("A99")
						|| Report_no.equals("A08") || Report_no.equals("A09") || Report_no.equals("A10")
						|| Report_no.equals("A12") || Report_no.equals("A13") || Report_no.equals("A15")) {
					...
					if (Report_no.equals("F01")) {
						...
						actMsg = insertA06(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, bank_type, Report_no,
								userid, username);
					...
					if (Report_no.equals("A08")) {
						actMsg = insertA08(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, bank_type, Report_no,
								userid, username);
					}
					if (Report_no.equals("A09")) {//96.12.10 add A09 by 2295
						actMsg = insertA09(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, bank_type, Report_no,
								userid, username);
					}
					if (Report_no.equals("A10")) {//97.06.13 add A10 by 2295
						actMsg = insertA10(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, bank_type, Report_no,
								userid, username);
					}
					if (Report_no.equals("A12")) {
						actMsg = insertA12(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, bank_type, Report_no,
								userid, username);
						...
						actMsg = insertA01_A05(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, bank_type, Report_no,
								userid, username);
					} else if (Report_no.equals("A15")) {//109.07.10 add A15 by 6493
						actMsg = insertA01_A05(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, bank_type, Report_no,
								userid, username);
					}
					...
					webURL_Y = StatusPgName + "?act=Status&Report_no=" + Report_no + "&bank_code=" + bank_code
							+ "&test=nothing";
					webURL_N = StatusPgName + "?act=Status&Report_no=" + Report_no + "&bank_code=" + bank_code
							+ "&test=nothing";
					request.setAttribute("actMsg", actMsg);
					request.setAttribute("alertMsg", alertMsg);
					request.setAttribute("webURL_Y", webURL_Y);
			...
			rd = application.getRequestDispatcher(nextPgName
					+ "?goPages=WMFileEdit_Status.jsp&act=Status&Report_no=" + Report_no + "&bank_code="
					+ bank_code + "&S_YEAR=" + S_YEAR + "&S_MONTH=" + S_MONTH + "&bank_type=" + bank_type);
		} else if (act.equals("Update")) {
			if (Report_no.equals("A01") || Report_no.equals("A02") || Report_no.equals("A03")
					|| Report_no.equals("A04") || Report_no.equals("A05")) {
				actMsg = updateA01_A05(request, String.valueOf(Integer.parseInt(S_YEAR)),
						String.valueOf(Integer.parseInt(S_MONTH)), bank_code, Report_no, userid, username);
			} else if (Report_no.equals("B01")) {
				...
				actMsg = updateB03(request, S_YEAR, S_MONTH, bank_code, Report_no, userid, username);
					...
					|| Report_no.equals("A08") || Report_no.equals("A09") || Report_no.equals("A10")
					|| Report_no.equals("A12") || Report_no.equals("A13") || Report_no.equals("A15")) {//94.11.11 add by 2295  //96.12.03 add by 2295 //97.06.13 //104.01.08
				if (!CheckFileLock(S_YEAR, S_MONTH, bank_code, Report_no)) {//98.04.20 add 該檔沒有被LOCK住
					if (Report_no.equals("F01")) {
						...
						actMsg = updateA01_A05(request, S_YEAR, S_MONTH, bank_code, Report_no, userid,
								username);
					}
					if (Report_no.equals("A08")) {
						actMsg = updateA08(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, Report_no, userid,
								username);
					}
					if (Report_no.equals("A09")) {
						actMsg = updateA09(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, Report_no, userid,
								username);
					}
					if (Report_no.equals("A10")) {//97.06.13 add A10 by 2295
						actMsg = updateA10(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, Report_no, userid,
								username);
					}
					if (Report_no.equals("A12")) {
						actMsg = updateA12(request, String.valueOf(Integer.parseInt(S_YEAR)),
								String.valueOf(Integer.parseInt(S_MONTH)), bank_code, Report_no, userid,
								username);
					} else if (Report_no.equals("A13")) { //104.10.12 add A13 by 2295
						actMsg = updateA01_A05(request, S_YEAR, S_MONTH, bank_code, Report_no, userid,
			...
			if (Report_no.equals("A01") || Report_no.equals("A02") || Report_no.equals("A03")
					|| Report_no.equals("A04") || Report_no.equals("A05") || Report_no.equals("A06")
					|| Report_no.equals("A99") || Report_no.equals("A08") || Report_no.equals("A09")
					|| Report_no.equals("A10") || Report_no.equals("A12") || Report_no.equals("F01")
					|| Report_no.equals("A13") || Report_no.equals("A15") || Report_no.equals("B01")
					|| Report_no.equals("B02") || Report_no.equals("M01") || Report_no.equals("M02")
					|| Report_no.equals("M04") || Report_no.equals("M06") || Report_no.equals("M07")
					|| Report_no.equals("M08")) {//95.04.14 add A06 by 2295//95.05.18 add A99 by 2295//96.12.03 add A09 by 2295 //97.06.13 add A10 by 2295
				actMsg = deleteA01_A10_B01_B02_M01_M08_F01(request, String.valueOf(Integer.parseInt(S_YEAR)),
						String.valueOf(Integer.parseInt(S_MONTH)), bank_code, Report_no, userid, username);
			} else if (Report_no.equals("B03")) {
...
<%!private final static String report_no = "WMFileEdit";
	...
	//109.07.10 add A15 by 6493
	//110.08.16 add A99.amt_name by 2295[111.06.13取消]
	public String insertA01_A05(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String bank_type, String Report_no, String userid, String username) throws Exception {
		System.out.println("insertA01_A05 begin");		
		String[] Amt = request.getParameterValues("amt");
		String[] MonthAmt = request.getParameterValues("month_amt");
		String[] YearAmt = request.getParameterValues("year_amt");
		...
		String amtType990422 = (String) request.getParameter("amtType990422");//110.03.23
		String amtText990422 = (String) request.getParameter("amtText990422");//110.03.23
		String txt990623 = (String) request.getParameter("txt990623");//110.03.23
		String amtType990623 = (String) request.getParameter("amtType990623");//110.03.23
		String amtText990623 = (String) request.getParameter("amtText990623");//110.03.23
		String txt990624 = (String) request.getParameter("txt990624");//110.03.23
		String txt9906251 = (String) request.getParameter("txt9906251");//110.03.23
		String txt9906252 = (String) request.getParameter("txt9906252");//110.03.23
		String txt990626 = (String) request.getParameter("txt990626");//110.03.23
		String txt9906261 = (String) request.getParameter("txt9906261");//110.03.23
		String txt9906262 = (String) request.getParameter("txt9906262");//110.03.23
		String txt991311 = request.getParameter("txt991311");//111.08.24
		String txt991321 = request.getParameter("txt991321");//111.08.24
		...
		try {
			...
			if (data.size() != 0) {
				...
				if (Report_no.equals("A05")) {//102.01.15 add by 2295
					updateDBSqlList.add("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?)");
					System.out.println("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?)");
				} else if(Report_no.equals("A02")){//110.03.19 add by 6493
					updateDBSqlList.add("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?,?,?)");
					System.out.println("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?,?,?)");					
				} else if (Report_no.equals("A15")) {//109.07.10 add by 6493
					updateDBSqlList.add("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?,?,?,SYSTIMESTAMP)");
					System.out.println("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?,?,?,SYSTIMESTAMP)");
				/*} else if (Report_no.equals("A99")) {//110.08.16 add by 2295[111.06.13取消]	
					updateDBSqlList.add("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?)");
					System.out.println("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?)");
				*/	
				} else {
				...
				for (int i = 0; i < Acc_Code.length; i++) {
					...
					System.out.println("S_YEAR="+S_YEAR+":S_MONTH="+S_MONTH+":bank_code="+bank_code+":Acc_Code="+Acc_Code[i].trim());
					if (Report_no.equals("A01") || Report_no.equals("A99")) {
						...
						if ("990421".equals(Acc_Code[i])) {
							...
							dataList.add("");//amt_name1
							dataList.add("");//amt_name2
							...
							if("1".equals(amtType990422)){
								dataList.add(amtText990422);//amt_name1
								dataList.add("");//amt_name2
							} else if("2".equals(amtType990422)){
								dataList.add("");//amt_name1
								dataList.add("不受限制");//amt_name2								
							}else{
								dataList.add("");//amt_name1
								dataList.add("");//amt_name2
							}
							...
							dataList.add("");//amt_name1
							dataList.add("");//amt_name2
						} else if ("990623".equals(Acc_Code[i])) {//104.02.13
							dataList.add(txt990623);//amt_name
							if("1".equals(amtType990623)){
								dataList.add(amtText990623);//amt_name1
								dataList.add("");//amt_name2
							} else if("2".equals(amtType990623)){
								dataList.add("");//amt_name1
								dataList.add("不受限制");//amt_name2								
							}else{
								dataList.add("");//amt_name1
						...
						} else if ("990624".equals(Acc_Code[i])) {//104.02.13
							dataList.add(txt990624);//amt_name
							dataList.add("");//amt_name1
							dataList.add("");//amt_name2
						} else if ("990625".equals(Acc_Code[i])) {//104.02.13
							dataList.add("");//amt_name
							if("1".equals(Amt[i])){
								dataList.add(txt9906251);//amt_name1
								dataList.add(txt9906252);//amt_name2
							}else{								
								dataList.add("");//amt_name1
								dataList.add("");//amt_name2
							}
						} else if ("990626".equals(Acc_Code[i])) {//104.02.13
							if("1".equals(Amt[i])){
								dataList.add(txt990626);//amt_name
								dataList.add("");//amt_name1
								dataList.add("");//amt_name2
							}else if("2".equals(Amt[i])){
								dataList.add(txt990626);//amt_name
								dataList.add(txt9906261);//amt_name1
								dataList.add(txt9906262);//amt_name2
							}else if("4".equals(Amt[i])){
								dataList.add("");//amt_name
								dataList.add(txt9906261);//amt_name1
								dataList.add(txt9906262);//amt_name2
							}else{								
						...
						} else if ("991311".equals(Acc_Code[i])) {//111.08.24 add
							dataList.add(txt991311);//amt_name
							dataList.add("");//amt_name1
							dataList.add("");//amt_name2	
						} else if ("991321".equals(Acc_Code[i])) {//111.08.24 add
							dataList.add(txt991321);//amt_name
							dataList.add("");//amt_name1
							dataList.add("");//amt_name2		
						} else {
							dataList.add("");//amt_name
				   ...
				   /*		
				   	    ...
				   	    if ("992451".equals(Acc_Code[i]) || "992453".equals(Acc_Code[i]) || "992455".equals(Acc_Code[i]) || "992461".equals(Acc_Code[i]) || "992463".equals(Acc_Code[i]) || "992465".equals(Acc_Code[i])) {
				   	    	dataList.add("");//amt
				   	    	dataList.add(Amt[i]);//amt_name
				   	    	System.out.println("amt_name="+Amt[i]);
				   	    }else{
				   	    	dataList.add(Utility.setNoCommaFormat(Amt[i]));//amt
				   	    	dataList.add("");//amt_name
				   	    	System.out.println("amt="+Utility.setNoCommaFormat(Amt[i]));
				   	    }		
				   	*/    
					...
					} else if (Report_no.equals("A15")) {//109.07.10 add
						dataList.add(Utility.setNoCommaFormat( "".equals(MonthAmt[i])?"0":MonthAmt[i]));//month_amt							  
						dataList.add(Utility.setNoCommaFormat( "".equals(YearAmt[i])?"0":YearAmt[i]));//year_amt
						dataList.add(userid);//user_id
						dataList.add(username);//user_name
					}
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					if (Report_no.equals("A01")) {
						...
						errMsg = errMsg + UpdateA01.doParserReport_A01(Report_no, S_YEAR, S_MONTH, "", bank_code, "M",
								"W", bank_type, userid, username, batch_no);
					} else if (Report_no.equals("A02")) {//add by winnin 2004.12.10
						Date today = new Date();
						int batch_no = today.hashCode();
						errMsg = errMsg + UpdateA02.doParserReport_A02(Report_no, S_YEAR, S_MONTH, "", bank_code, "M",
								"W", bank_type, userid, username, batch_no);
					} else if (Report_no.equals("A03")) {//add by winnin 2004.12.23
						Date today = new Date();
						int batch_no = today.hashCode();
						errMsg = errMsg + UpdateA03.doParserReport_A03(Report_no, S_YEAR, S_MONTH, "", bank_code, "M",
								"W", bank_type, userid, username, batch_no);
					} else if (Report_no.equals("A04")) {//add by winnin 2004.12.10
						Date today = new Date();
						int batch_no = today.hashCode();
						errMsg = errMsg + UpdateA04.doParserReport_A04(Report_no, S_YEAR, S_MONTH, "", bank_code, "M",
								"W", bank_type, userid, username, batch_no);
					} else if (Report_no.equals("A05")) {//add by winnin 2004.12.10
						Date today = new Date();
						int batch_no = today.hashCode();
						errMsg = errMsg + UpdateA05.doParserReport_A05(Report_no, S_YEAR, S_MONTH, "", bank_code, "M",
								"W", bank_type, userid, username, batch_no);
					} else if (Report_no.equals("A99")) {//add by 2295 2005.5.18
						Date today = new Date();
						int batch_no = today.hashCode();
						errMsg = errMsg + UpdateA99.doParserReport_A99(Report_no, S_YEAR, S_MONTH, "", bank_code, "M",
								"W", bank_type, userid, username, batch_no);
						...
						Date today = new Date();
						int batch_no = today.hashCode();
						errMsg = errMsg + UpdateA13.doParserReport_A13(Report_no, S_YEAR, S_MONTH, "", bank_code, "M",
								"W", bank_type, userid, username, batch_no);
						if (errMsg.startsWith("U")) {//檢核成功
							errMsg = "U:相關資料寫入資料庫成功";
						}
					} else if (Report_no.equals("A15")) {//add by 6493 109.07.10
						Date today = new Date();
						int batch_no = today.hashCode();
						errMsg = UpdateA15.doParserReport_A15(Report_no, S_YEAR, S_MONTH, "", bank_code, "M",
								"W", bank_type, userid, username, batch_no);
						if (errMsg.startsWith("U")) {//檢核成功
							errMsg = "U:相關資料寫入資料庫成功";
						}
	...
	public String insertA06(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String bank_type/*93.12.18 add by 2295*/, String Report_no, String userid, String username)
			throws Exception {
		...
		try {
			...
			List data = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,amt_3month,amt_6month,amt_1year,amt_2year,amt_over2year,amt_total");
			System.out.println("" + Report_no + ".size=" + data.size());
			...
			if (data.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateA06.doParserReport_A06(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
					if (errMsg.startsWith("U")) {//檢核成功
						errMsg = "U:相關資料寫入資料庫成功";
					}
				} else {
	...
	public String insertB01(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		...
		String bank_type = (request.getAttribute("bank_type") == null) ? "3"
				: (String) request.getAttribute("bank_type");
		String errMsg = "";
		StringBuffer sqlCmd = new StringBuffer();
		List paramList = new ArrayList();
		List updateDBList = new ArrayList();//0:sql 1:data
		...
		try {
			...
			if (data1.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateB01.doParserReport_B01(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
				} else {
					errMsg = errMsg + "相關資料寫入資料庫失敗<br>[DBManager.getErrMsg()]:<br>" + DBManager.getErrMsg();
				}
	...
	public String insertB02(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		...
		try {
			...
			if (data1.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateB02.doParserReport_B02(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
				} else {
					errMsg = errMsg + "相關資料寫入資料庫失敗<br>[DBManager.getErrMsg()]:<br>" + DBManager.getErrMsg();
				}
	...
	public String insertB03(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String errMsg = "";
		//B03_1
		...
		try {
			...
			if (data1.size() != 0) {
				...
				if (funs_master_no_1 == null)
					System.out.println("funs_master_no_1	is null");
				if (funs_sub_no_1 == null)
					System.out.println("funs_sub_no_1	is null");
				if (funs_next_no_1 == null)
					System.out.println("funs_master_no_1	is null");
				if (loan_cnt_totacc == null)
					System.out.println("funs_next_no_1	is null");
				if (loan_amt_totacc_fund == null)
					System.out.println("loan_amt_totacc_fund	is null");
				if (loan_amt_totacc_bank == null)
					System.out.println("loan_amt_totacc_bank	is null");
				if (loan_amt_totacc_tot == null)
					System.out.println("loan_amt_totacc_tot	is null");
				if (loan_cnt_bal == null)
					System.out.println("loan_cnt_bal	is null");
				if (loan_amt_bal_fund == null)
					System.out.println("loan_amt_bal_fund	is null");
				if (loan_amt_bal_bank == null)
					System.out.println("loan_amt_bal_bank	is null");
				if (loan_amt_bal_tot == null)
					System.out.println("loan_amt_bal_tot	is null");
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateB03.doParserReport_B03(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
				} else {
					errMsg = errMsg + "相關資料寫入資料庫失敗";
				}
	...
	public String insertM01(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		...
		//年份資料表示轉換93-->093
		String s_year = "0000" + S_YEAR;
		s_year = s_year.substring(s_year.length() - 3);
		String s_month = "0000" + S_MONTH;
		s_month = s_month.substring(s_month.length() - 2);
		System.out.println("轉換後s_year =[" + s_year + "]");
		System.out.println("轉換後s_month=[" + s_month + "]");

		String bank_type = (request.getAttribute("bank_type") == null) ? "4"
				: (String) request.getAttribute("bank_type");
		String errMsg = "";
		StringBuffer sqlCmd = new StringBuffer();
		List paramList = new ArrayList();
		...
		try {
			...
			if (data1 == null || data1.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateM01.doParserReport_M01(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
				} else {
					errMsg = errMsg + "相關資料寫入資料庫失敗<br>[DBManager.getErrMsg()]:<br>" + DBManager.getErrMsg();
				}
	...
	//jei 931210
	public String insertM02(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		...
		try {
			...
			if (data1.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateM02.doParserReport_M02(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
				} else {
					errMsg = errMsg + "相關資料寫入資料庫失敗<br>[DBManager.getErrMsg()]:<br>" + DBManager.getErrMsg();
				}
	...
	public String insertM03(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String errMsg = "";
		...
		try {
			...
			if (data1.size() != 0 || data2.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateM03.doParserReport_M03(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
				} else {
					errMsg = errMsg + "相關資料寫入資料庫失敗<br>[DBManager.getErrMsg()]:<br>" + DBManager.getErrMsg();
				}
	...
	//jei 931212
	public String insertM04(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		...
		try {
			...
			if (data1.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateM04.doParserReport_M04(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
				} else {
					errMsg = errMsg + "相關資料寫入資料庫失敗<br>[DBManager.getErrMsg()]:<br>" + DBManager.getErrMsg();
				}
	...
	public String insertM05(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		...
		try {
			...
			if (data1.size() != 0 || data2.size() != 0 || data3.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateM05.doParserReport_M05(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
				} else {
					errMsg = errMsg + "相關資料寫入資料庫失敗";
				}
	...
	public String insertM06_M07(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String errMsg = "";
			...
			if (data1.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					if (Report_no.equals("M06")) {
						Date today = new Date();
						int batch_no = today.hashCode();
						errMsg = errMsg + UpdateM06.doParserReport_M06(Report_no, S_YEAR, S_MONTH, "", bank_code, "M",
								"W", bank_type, userid, username, batch_no);
					} else {
						Date today = new Date();
						int batch_no = today.hashCode();
						errMsg = errMsg + UpdateM07.doParserReport_M07(Report_no, S_YEAR, S_MONTH, "", bank_code, "M",
								"W", bank_type, userid, username, batch_no);
					}
				} else {
					errMsg = errMsg + "相關資料寫入資料庫失敗";
	...
	public String insertM08(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String errMsg = "";
		...
		try {
			...
			if (data1.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateM08.doParserReport_M08(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
				} else {
					errMsg = errMsg + "相關資料寫入資料庫失敗";
				}
	...
	//94.11.10 add by 2295
	public String insertF01(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		...
		try {
			...
			if (dbData != null && (!(((DataObject) dbData.get(0)).getValue("count")).toString().equals("0"))) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					errMsg = errMsg + UpdateF01.doParserReport_F01(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
	...
	public String insertA08(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String bank_type, String Report_no, String userid, String username) throws Exception {
		...
		List dataList = new LinkedList();//儲存參數的data
		try {
			WarnAccount_Cnt = (request.getParameter("WarnAccount_Cnt") == null) ? "0"
					: (String) request.getParameter("WarnAccount_Cnt");
			LimitAccount_Cnt = (request.getParameter("LimitAccount_Cnt") == null) ? "0"
					: (String) request.getParameter("LimitAccount_Cnt");
			ErrorAccount_Cnt = (request.getParameter("ErrorAccount_Cnt") == null) ? "0"
					: (String) request.getParameter("ErrorAccount_Cnt");
			OtherAccount_Cnt = (request.getParameter("OtherAccount_Cnt") == null) ? "0"
					: (String) request.getParameter("OtherAccount_Cnt");
			DepositAccount_TCnt = (request.getParameter("DepositAccount_TCnt") == null) ? "0"
					: (String) request.getParameter("DepositAccount_TCnt");
			...
			if (data.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateA08.doParserReport_A08(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
					if (errMsg.startsWith("U")) {//檢核成功
						errMsg = "U:相關資料寫入資料庫成功";
					}
	...
	public String insertA09(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String bank_type, String Report_no, String userid, String username) throws Exception {
		...
		try {
			...
			PUSH_over_amt = (request.getParameter("PUSH_over_amt") == null) ? "0"
					: (String) request.getParameter("PUSH_over_amt");//剩餘金額-催收款(B)
			totalamt = (request.getParameter("totalamt") == null) ? "0" : (String) request.getParameter("totalamt");//全會放出總金額(C)
			PUSH_totalamt = (request.getParameter("PUSH_totalamt") == null) ? "0"
					: (String) request.getParameter("PUSH_totalamt");//全會放出總金額-催收款(D)
			Over_total_rate = (request.getParameter("Over_total_rate") == null) ? "0"
					: (String) request.getParameter("Over_total_rate");//佔放款總額的比率(A-B)/(C-D)
			...
			if (data.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateA09.doParserReport_A09(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
					if (errMsg.startsWith("U")) {//檢核成功
						errMsg = "U:相關資料寫入資料庫成功";
					}
	...
	public String insertA10(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String bank_type, String Report_no, String userid, String username) throws Exception {
		...
		String property_loss = "";//非授信資產可能遭受損失
		StringBuffer sqlCmd = new StringBuffer();
		List paramList = new ArrayList();
		String errMsg = "";
		int zerodata = 0;
		...
		try {
			...
			invest1_amt = (request.getParameter("invest1_amt") == null) ? "0"
					: (String) request.getParameter("invest1_amt");//投資-第一類
			invest2_amt = (request.getParameter("invest2_amt") == null) ? "0"
					: (String) request.getParameter("invest2_amt");//投資-第二類
			invest3_amt = (request.getParameter("invest3_amt") == null) ? "0"
					: (String) request.getParameter("invest3_amt");//投資-第三類
			invest4_amt = (request.getParameter("invest4_amt") == null) ? "0"
					: (String) request.getParameter("invest4_amt");//投資-第四類
			other1_amt = (request.getParameter("other1_amt") == null) ? "0"
					: (String) request.getParameter("other1_amt");//其他-第一類
			other2_amt = (request.getParameter("other2_amt") == null) ? "0"
					: (String) request.getParameter("other2_amt");//其他-第二類
			other3_amt = (request.getParameter("other3_amt") == null) ? "0"
					: (String) request.getParameter("other3_amt");//其他-第三類
			other4_amt = (request.getParameter("other4_amt") == null) ? "0"
					: (String) request.getParameter("other4_amt");//其他-第四類
			loan1_baddebt = (request.getParameter("loan1_baddebt") == null) ? "0"
					: (String) request.getParameter("loan1_baddebt");//放款-帳列備抵呆帳-第一類
			loan2_baddebt = (request.getParameter("loan2_baddebt") == null) ? "0"
					: (String) request.getParameter("loan2_baddebt");//放款-帳列備抵呆帳-第二類
			loan3_baddebt = (request.getParameter("loan3_baddebt") == null) ? "0"
					: (String) request.getParameter("loan3_baddebt");//放款-帳列備抵呆帳-第三類
			loan4_baddebt = (request.getParameter("loan4_baddebt") == null) ? "0"
					: (String) request.getParameter("loan4_baddebt");//放款-帳列備抵呆帳-第四類
			build1_baddebt = (request.getParameter("build1_baddebt") == null) ? "0"
					: (String) request.getParameter("build1_baddebt");//建築貸款-帳列備抵呆帳-第一類
			build2_baddebt = (request.getParameter("build2_baddebt") == null) ? "0"
					: (String) request.getParameter("build2_baddebt");//建築貸款-帳列備抵呆帳-第二類
			build3_baddebt = (request.getParameter("build3_baddebt") == null) ? "0"
					: (String) request.getParameter("build3_baddebt");//建築貸款-帳列備抵呆帳-第三類
			build4_baddebt = (request.getParameter("build4_baddebt") == null) ? "0"
					: (String) request.getParameter("build4_baddebt");//建築貸款-帳列備抵呆帳-第四類
			baddebt_flag = Utility.getTrimString(request.getParameter("baddebt_flag"));//備呆等於或大於應提最低標準
			//if("N".equals(baddebt_flag)){
			baddebt_noenough = (request.getParameter("baddebt_noenough") == null
					|| ((String) request.getParameter("baddebt_noenough")).equals("")) ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_noenough"));//建築貸款-帳列備抵呆帳-第四類;//備呆不足額
			baddebt_delay = Utility.getTrimString(request.getParameter("baddebt_delay"));//申請展延
			baddebt_104 = (request.getParameter("baddebt_104") == null
					|| ((String) request.getParameter("baddebt_104")).equals("")) ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_104"));//104年底前提足備呆
			baddebt_105 = (request.getParameter("baddebt_105") == null
					|| ((String) request.getParameter("baddebt_105")).equals("")) ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_105"));//105年底前提足備呆
			baddebt_106 = (request.getParameter("baddebt_106") == null
					|| ((String) request.getParameter("baddebt_106")).equals("")) ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_106"));//106年底前提足備呆
			baddebt_107 = (request.getParameter("baddebt_107") == null
					|| ((String) request.getParameter("baddebt_107")).equals("")) ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_107"));//107年底前提足備呆
			baddebt_108 = (request.getParameter("baddebt_108") == null
					|| ((String) request.getParameter("baddebt_108")).equals("")) ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_108"));//108年底前提足備呆
			//}
			property_loss = (request.getParameter("property_loss") == null
					|| ((String) request.getParameter("property_loss")).equals("")) ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("property_loss"));//非授信資產可能遭受損失
			...
			System.out.println("property_loss=" + property_loss);
			sqlCmd.append("SELECT * FROM " + Report_no + " WHERE m_year=? AND m_month=? AND bank_code=?");
			paramList.add(S_YEAR);
			paramList.add(S_MONTH);
			paramList.add(bank_code);
			...
			if (data.size() != 0) {
				...
				updateDBSqlList.add("INSERT INTO " + Report_no
						+ " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
				dataList = new LinkedList();
				dataList.add(S_YEAR);//m_year
				dataList.add(S_MONTH);//m_month
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateA10.doParserReport_A10(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
					if (errMsg.startsWith("U")) {//檢核成功
						errMsg = "U:相關資料寫入資料庫成功";
					}
	...
	public String insertA12(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String bank_type, String Report_no, String userid, String username) throws Exception {
		...
		List dataList = new LinkedList();//儲存參數的data
		try {
			baddebt_Amt = (request.getParameter("baddebt_Amt") == null) ? "0"
					: (String) request.getParameter("baddebt_Amt");
			loss_Amt = (request.getParameter("loss_Amt") == null) ? "0" : (String) request.getParameter("loss_Amt");
			profit_Amt = (request.getParameter("profit_Amt") == null) ? "0"
					: (String) request.getParameter("profit_Amt");
			...
			if (data.size() != 0) {
				...
				if (DBManager.updateDB_ps(updateDBList)) {
					...
					Date today = new Date();
					int batch_no = today.hashCode();
					errMsg = errMsg + UpdateA12.doParserReport_A12(Report_no, S_YEAR, S_MONTH, "", bank_code, "M", "W",
							bank_type, userid, username, batch_no);
	...
	//109.07.10 add A15 by 6493
	//110.08.16 add A99.amt_name by 2295[111.06.14取消]
	public String updateA01_A05(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String[] Amt = request.getParameterValues("amt");
		String[] MonthAmt = request.getParameterValues("month_amt");
		String[] YearAmt = request.getParameterValues("year_amt");
		...
		String txt9906261 = (String) request.getParameter("txt9906261");//110.03.23
		String txt9906262 = (String) request.getParameter("txt9906262");//110.03.23
		String bank_type = (request.getAttribute("bank_type") == null) ? "6"
				: (String) request.getAttribute("bank_type");
		String txt991311 = (String) request.getParameter("txt991311");//111.08.24
		String txt991321 = (String) request.getParameter("txt991321");//111.08.24
		...
		try {
			...
			if (data.size() == 0) {
				...
				if (Report_no.equals("A05")) {
					...
					sqlCmd.append(" month_amt,year_amt,null,null,null ");
				} else {
					sqlCmd.append(" amt");
				}
				...
				/*
				if (Report_no.equals("A99")) {
					sqlCmd.append(" ,amt_name");//110.08.16 add by 2295[111.06.14取消]
				}
				*/
				...
				if (Report_no.equals("A05")) {//102.01.15 add by 2295
					...
					sqlCmd.append("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?,?,?,SYSTIMESTAMP )");
					System.out.println("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?,?,?,SYSTIMESTAMP )");
				/*	
				} else if(Report_no.equals("A99")){//110.08.16 add by 2295[111.06.14取消]	
					sqlCmd.append("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?)");
					System.out.println("INSERT INTO " + Report_no + " VALUES(?,?,?,?,?,?)");
				*/	
				} else {
	...
	public String updateA06(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String[] Amt_3month = request.getParameterValues("amt_3month");
		String[] Amt_6month = request.getParameterValues("amt_6month");
		String[] Amt_1year = request.getParameterValues("amt_1year");
	...
	public String updateA08(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String WarnAccount_Cnt = "";//警示帳戶」戶數（A）
		String LimitAccount_Cnt = "";//「衍生管制帳戶」戶數（B）
		String ErrorAccount_Cnt = "";//「自行篩選有異常並已採資金流出管制措施之存款帳戶」戶數（不包括警示帳戶及衍生管制帳戶）（C）
		...
		try {
			...
			if (data.size() == 0) {
				...
				sqlCmd.append(" INSERT INTO " + Report_no + "_LOG ");
				sqlCmd.append(
						" select m_year,m_month,bank_code,warnAccount_cnt,limitAccount_cnt,erroraccount_cnt,otheraccount_cnt,depositaccount_tcnt");
				sqlCmd.append(",?,?,sysdate,'U'");
				sqlCmd.append(" from " + Report_no);
				sqlCmd.append(" WHERE m_year=? AND m_month=? AND bank_code=?");
				dataList.add(userid);
	...
	public String updateA09(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String over_cnt = "";//剩餘件數
		String over_amt = "";//剩餘金額(A)
		String PUSH_over_amt = "";//剩餘金額-催收款(B)
		...
		try {
			...
			if (data.size() == 0) {
				...
				sqlCmd.append(" INSERT INTO " + Report_no + "_LOG ");
				sqlCmd.append(
						" select m_year,m_month,bank_code,over_cnt,over_amt,PUSH_over_amt,totalamt,PUSH_totalamt,Over_total_rate");
	...
	public String updateA10(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String loan1_amt = "";//放款-第一類
		String loan2_amt = "";//放款-第二類
		String loan3_amt = "";//放款-第三類
		...
		try {
			...
			baddebt_flag = Utility.getTrimString(request.getParameter("baddebt_flag"));//備呆等於或大於應提最低標準
			//if("N".equals(baddebt_flag)){
			baddebt_noenough = (request.getParameter("baddebt_noenough") == null) ? "0"
					: Utility.setNoCommaFormat((String) request.getParameter("baddebt_noenough"));//建築貸款-帳列備抵呆帳-第四類;//備呆不足額
			baddebt_delay = Utility.getTrimString(request.getParameter("baddebt_delay"));//申請展延
			baddebt_104 = (request.getParameter("baddebt_104") == null
					|| Utility.setNoCommaFormat((String) request.getParameter("baddebt_104")) == "") ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_104"));//104年底前提足備呆
			baddebt_105 = (request.getParameter("baddebt_105") == null
					|| Utility.setNoCommaFormat((String) request.getParameter("baddebt_105")) == "") ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_105"));//105年底前提足備呆
			baddebt_106 = (request.getParameter("baddebt_106") == null
					|| Utility.setNoCommaFormat((String) request.getParameter("baddebt_106")) == "") ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_106"));//106年底前提足備呆
			baddebt_107 = (request.getParameter("baddebt_107") == null
					|| Utility.setNoCommaFormat((String) request.getParameter("baddebt_107")) == "") ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_107"));//107年底前提足備呆
			baddebt_108 = (request.getParameter("baddebt_108") == null
					|| Utility.setNoCommaFormat((String) request.getParameter("baddebt_108")) == "") ? "0"
							: Utility.setNoCommaFormat((String) request.getParameter("baddebt_108"));//108年底前提足備呆
			//}
			property_loss = (request.getParameter("property_loss") == null) ? "0"
					: (String) request.getParameter("property_loss");
			...
			List data = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,loan1_amt,loan2_amt,loan3_amt,loan4_amt,invest1_amt,invest2_amt,invest3_amt,invest4_amt,other1_amt,other2_amt,other3_amt,other4_amt"
							...
							+ ",baddebt_flag,baddebt_noenough,baddebt_delay,baddebt_104,baddebt_105,baddebt_106,baddebt_107,baddebt_108,property_loss");
			System.out.println("" + Report_no + ".size=" + data.size());
			System.out.println("loan1_amt=" + loan1_amt);
			System.out.println("loan2_amt=" + loan2_amt);
			System.out.println("loan3_amt=" + loan3_amt);
			...
			if (data.size() == 0) {
				...
				sqlCmd.append(" INSERT INTO " + Report_no + "_LOG ");
				sqlCmd.append(
						" select m_year,m_month,bank_code,loan1_amt,loan2_amt,loan3_amt,loan4_amt,invest1_amt,invest2_amt,invest3_amt,invest4_amt,other1_amt,other2_amt,other3_amt,other4_amt");
				sqlCmd.append(
						" ,loan1_baddebt,loan2_baddebt,loan3_baddebt,loan4_baddebt,build1_baddebt,build2_baddebt,build3_baddebt,build4_baddebt");
				sqlCmd.append(
						" ,baddebt_flag,baddebt_noenough,baddebt_delay,baddebt_104,baddebt_105,baddebt_106,baddebt_107,baddebt_108,property_loss");
				...
				dataList.add(baddebt_107);
				dataList.add(baddebt_108);
				dataList.add(Utility.setNoCommaFormat(property_loss));
				updateDBDataList.add(dataList);
	...
	public String updateA12(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String baddebt_Amt = "";//轉銷呆帳金額-本月減少備抵呆帳B1
		String loss_Amt = "";//轉銷呆帳金額-本月直接認列損失B2
		String profit_Amt = "";//存款準備率降低所增加盈餘(本月增加金額)B3
	...
	public String updateB01(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String[] fund_master_no = request.getParameterValues("fund_master_no_c");
		String[] fund_sub_no = request.getParameterValues("fund_sub_no_c");
		String[] fund_next_no = request.getParameterValues("fund_next_no_c");
	...
	public String updateB02(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String[] run_master_no = request.getParameterValues("run_master_no_c");
		String[] run_sub_no = request.getParameterValues("run_sub_no_c");
		String[] run_next_no = request.getParameterValues("run_next_no_c");
	...
	public String updateM01(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
	...
	//jei 931210
	public String updateM02(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String[] loan_unit_no = request.getParameterValues("loan_unit_no_c");
		String[] data_range = request.getParameterValues("data_range_c");
		String[] guarantee_cnt = request.getParameterValues("guarantee_cnt");
	...
	public String updateM03(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String errMsg = "";
		String bank_type = (request.getAttribute("bank_type") == null) ? "4"
				: (String) request.getAttribute("bank_type");
	...
	//jei 931212
	public String updateM04(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String[] loan_use_no = request.getParameterValues("loan_use_no_c");
		String[] guarantee_no_month = request.getParameterValues("guarantee_no_month");
		String[] guarantee_no_month_p = request.getParameterValues("guarantee_no_month_p");
		...
		try {
			...
			if (data1.size() == 0) {
				...
				sqlCmd.append(
						" select m_year,m_month,loan_use_no,guarantee_no_month,guarantee_no_month_p,loan_amt_month,");
				sqlCmd.append("        loan_amt_month_p,guarantee_amt_month,guarantee_amt_month_p,guarantee_no_year,");
				sqlCmd.append(
						"        guarantee_no_year_p,loan_amt_year,loan_amt_year_p,guarantee_amt_year,guarantee_amt_year_p,");
	...
	//add by winnin 2004.12.09
	public String updateM05(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String[] Loan_Unit_No = request.getParameterValues("loan_unit_no");
		String[] Period_No = request.getParameterValues("period_no");
		String[] Item_No = request.getParameterValues("item_no");
	...
	//Method modify by egg 93.12.14
	public String updateM06_M07(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		//資料宣告
		String[] area_no = request.getParameterValues("area_no");
		String[] guarantee_no_month = request.getParameterValues("guarantee_no_month");
		...
		try {
			...
			if (data1.size() == 0) {
				...
				sqlCmd.append(
						"        guarantee_no_year,guarantee_amt_year,loan_amt_year,guarantee_no_totacc,guarantee_amt_totacc,");
	...
	//Method modify by egg 93.12.16
	public String updateM08(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		//資料宣告
		String[] id_no = request.getParameterValues("id_no");
		String[] data_range = request.getParameterValues("data_range");
		...
		String bank_type = (request.getAttribute("bank_type") == null) ? ""
				: (String) request.getAttribute("bank_type");
		...
		try {
			...
			if (data1.size() == 0) {
				...
				sqlCmd.append(" INSERT INTO " + Report_no + "_LOG ");
				sqlCmd.append(
						" select m_year,m_month,id_no,data_range,guarantee_no_month,loan_amt_month,guarantee_amt_month,");
	...
	//Method modify by egg 93.12.14
	public String updateB03(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String errMsg = "";
		...
		try {
			...
			if (data1.size() == 0 || data2.size() == 0 || data3.size() == 0 || data4.size() == 0) {
				...
				sqlCmd.append(
						" select m_year, m_month, funs_master_no, funs_sub_no, funs_next_no, loan_amt_bal, loan_amt_over, loan_rate_over,");
				sqlCmd.append(" ?,?,sysdate,'U'");
				...
				sqlCmd.append(
						" select m_year, m_month, funo_master_no, funo_sub_no, funo_next_no, funo_amt, funo_rate");
				sqlCmd.append(" ,?,?,sysdate,'U'");
	...
	public String updateF01(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String errMsg = "";
		String titleidx[] = { "A", "B", "C", "D", "E" };
		StringBuffer sqlCmd = new StringBuffer();
		...
		try {
			...
			if (dbData.size() == 0) {
				...
				sqlCmd.append(
						" select m_year,m_month,bank_code,dep_type,acct_type,acct_cnt_tm,bal_lm,dep_tm,wtd_tm,bal_tm");
				sqlCmd.append("        ,?,?,sysdate,'U'");
	...
	//109.07.10 add A15 by 6493
	public String deleteA01_A10_B01_B02_M01_M08_F01(HttpServletRequest request, String S_YEAR, String S_MONTH,
			String bank_code, String Report_no, String userid, String username) throws Exception {
		...
		try {
			...
			List data = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,add_date,batch_no,update_date");
			...
			if (data.size() == 0) {
				...
				sqlCmd.append(
						"DELETE FROM WML01 WHERE m_year=? AND m_month=? AND bank_code=? AND report_no=? AND input_method=?");
				...
				if (Report_no.equals("A10")) {
					...
					List WML03data = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
							"m_year,m_month,add_date,batch_no,update_date");
					...
					if (WML03data.size() > 0) {
						...
						sqlCmd.append(
								"select m_year,m_month,bank_code,report_no,serial_no,remark,user_id,user_name,update_date,?,?,sysdate,'D' from WML03  ");
				...
				if (data.size() == 0) {
					...
					if (Report_no.equals("A08")) {
						...
						sqlCmd.append(" select m_year,m_month,bank_code,acc_code,month_amt,year_amt,null,null,null");
					} else if (Report_no.equals("A05")) {
						sqlCmd.append(" select m_year,m_month,bank_code,acc_code,amt,amt_name");
					} else if (Report_no.equals("A06")) {//95.04.11 add by 2295
						sqlCmd.append(
								" select m_year,m_month,bank_code,acc_code,amt_3month,amt_6month,amt_1year,amt_2year,amt_over2year,amt_total");
					} else if (Report_no.equals("B01")) {
						sqlCmd.append(
								" select m_year,m_month,fund_master_no,fund_sub_no,fund_next_no,budget_amt,credit_pay_amt,credit_pay_rate,remark ");
					} else if (Report_no.equals("B02")) {
						sqlCmd.append(
								" select m_year,m_month,run_master_no,run_sub_no,run_next_no,loan_cnt_year,loan_amt_year,loan_cnt_totacc,loan_amt_totacc,loan_cnt_bal,loan_amt_bal_subtot,loan_amt_bal_fund,loan_amt_bal_bank,");
					} else if (Report_no.equals("M01")) {
						sqlCmd.append(" select m_year,m_month,guarantee_item_no,data_range,guarantee_cnt,loan_amt,");
						sqlCmd.append(
								"        guarantee_amt,loan_bal,guarantee_bal,over_notpush_cnt,over_notpush_bal,");
						sqlCmd.append("        over_okpush_cnt,over_okpush_bal,repay_tot_cnt,repay_tot_amt,");
						sqlCmd.append("        repay_bal_cnt,repay_bal_amt ");
					} else if (Report_no.equals("M02")) {
						sqlCmd.append(" select m_year,m_month,loan_unit_no,data_range,guarantee_cnt,loan_amt,");
						...
						sqlCmd.append(
								" select m_year,m_month,loan_use_no,guarantee_no_month,guarantee_no_month_p,loan_amt_month,");
						sqlCmd.append(
								"        loan_amt_month_p,guarantee_amt_month,guarantee_amt_month_p,guarantee_no_year,");
						sqlCmd.append(
								"        guarantee_no_year_p,loan_amt_year,loan_amt_year_p,guarantee_amt_year,guarantee_amt_year_p,");
						sqlCmd.append(
								"        guarantee_no_totacc,guarantee_no_totacc_p,loan_amt_totacc,loan_amt_totacc_p,");
						sqlCmd.append("        guarantee_amt_totacc,guarantee_amt_totacc_p ");
					} else if (Report_no.equals("M06") || Report_no.equals("M07")) {
						sqlCmd.append(
								" select m_year,m_month,area_no,guarantee_no_month,guarantee_amt_month,loan_amt_month,guarantee_no_year,");
						sqlCmd.append(
								"        guarantee_amt_year,loan_amt_year,guarantee_no_totacc,guarantee_amt_totacc,loan_amt_totacc,");
						sqlCmd.append("        guarantee_bal_no,guarantee_bal_amt,guarantee_bal_p,loan_bal ");
					} else if (Report_no.equals("M08")) {
						sqlCmd.append(
								" select m_year,m_month,id_no,data_range,guarantee_no_month,loan_amt_month,guarantee_amt_month,guarantee_bal_month,guarantee_bal_p");
					} else if (Report_no.equals("F01")) {
						sqlCmd.append(
								" select m_year,m_month,bank_code,dep_type,acct_type,acct_cnt_tm,bal_lm,dep_tm,wtd_tm,bal_tm");
					} else {
					...
					if (Report_no.equals("A02")) {//102.04.25 add a02.amt_name//111.03.18 add a02.amt_name1,amt_name2 
						sqlCmd.append(",?,?,sysdate,'D',amt_name,amt_name1,amt_name2");
					/*	
					} else if (Report_no.equals("A99")) {//111.03.18 add a99.amt_name[111.06.13取消]
	...
	public String deleteB03(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String errMsg = "";
		...
		try {
			...
			if (data.size() == 0) {
				...
				if (data.size() == 0) {
					...
					sqlCmd.append(" INSERT INTO " + Report_no + "_1_LOG ");
					sqlCmd.append(
							" select m_year, m_month, funs_master_no, funs_sub_no, funs_next_no, loan_cnt_totacc,");
					sqlCmd.append(" loan_amt_totacc_fund, loan_amt_totacc_bank, loan_amt_totacc_tot, loan_cnt_bal,");
					sqlCmd.append(" loan_amt_bal_fund, loan_amt_bal_bank, loan_amt_bal_tot");
					sqlCmd.append(" ,?,?,sysdate,'U'");
					sqlCmd.append(" from " + Report_no + "_1");
	...
	public String deleteM03(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String errMsg = "";
		String input_method = ""; //add by egg 2004.12.21
		...
		try {
			...
			if (data.size() == 0) {
				...
				if (data.size() == 0) {
					...
					sqlCmd.append(
							"        guarantee_amt_month,guarantee_cnt_year,loan_amt_year,guarantee_amt_year,guarantee_bal_totacc,");
					sqlCmd.append("        guarantee_bal_totacc_over,repay_bal_totacc");
					sqlCmd.append("        ,?,?,sysdate,'U'");
					sqlCmd.append("   from M03");
					sqlCmd.append(" WHERE m_year=? AND m_month=?");
	...
	//add by winnin 2004.12.09
	public String deleteM05(HttpServletRequest request, String S_YEAR, String S_MONTH, String bank_code,
			String Report_no, String userid, String username) throws Exception {
		String errMsg = "";
			...
			if (data.size() == 0) {
				...
				if (data.size() == 0) {
					...
					if (data.size() == 0) {
						...
						sqlCmd.append(" INSERT INTO M05_TOTACC_LOG ");
						sqlCmd.append(
								" select m_year,m_month,loan_unit_no,fix_no,guarantee_no_totacc,guarantee_amt_totacc");
						sqlCmd.append("        ,?,?,sysdate,'U'");
						sqlCmd.append("   from M05_TOTACC");
						sqlCmd.append(" WHERE m_year=? AND m_month=?");
						dataList.add(userid);
	...
	private List getData_A01_A05(String S_YEAR, String S_MONTH, String bank_code, String bank_type, String acc_div, boolean isInitial, boolean isMonth01, String Report_no) {
		List dbData = null;
		if(Report_no.equals("A15")){
			//依不同編輯情況，
			...
			//客戶數 |  初次申報		|	0							|	0
			//		|  修改初次申報	|	該月year_amt					|	0
			//		|  一月新增		|	上月year_amt					|	上月year_amt						(V)
			//		|  一月修改		|	上月year_amt+該月month_amt	|	上月year_amt						(V)
			//		|  其他新增		|	上月year_amt					|	上月year_amt						(V)
			//		|  其他修改		|	上月year_amt+該月month_amt	|	上月year_amt						(V)
			//------+---------------+-------------------------------+---------------------------------
			//交易數 |  初次申報		|	0							|	0
			//		|  修改初次申報	|	該月year_amt					|	0
			//		|  一月新增		|	0							|	0
			...
			List data_lastMonth = getLastMonthData(S_YEAR, S_MONTH, bank_code, Report_no, bank_type);
			List data_thisMonth = getData_A01_A05(S_YEAR, S_MONTH, bank_code, bank_type, acc_div);
			String lastMonhtAccCode = "",thisMonhtAccCode = "";
			boolean changeData = false; //符合需取得上個月資料者(上表標記V)是否已更換資料
			//非第一次申報
			if(!isInitial){
				for(int i=0; i<data_thisMonth.size(); i++){
					changeData = false;
					thisMonhtAccCode = (((DataObject)data_thisMonth.get(i)).getValue("acc_code")==null?"":((DataObject)data_thisMonth.get(i)).getValue("acc_code")).toString();
					//條件符合上表標記(V)者
					if( !(isMonth01 && !"00".equals(thisMonhtAccCode.substring(4))) ){
						//取得上個月資料
						for(int j=0; j<data_lastMonth.size(); j++){
							lastMonhtAccCode = (((DataObject)data_lastMonth.get(j)).getValue("acc_code")==null?"":((DataObject)data_lastMonth.get(j)).getValue("acc_code")).toString();
							if(lastMonhtAccCode.equals(thisMonhtAccCode)){
								changeData = true;
								//month_amt(取該月DB值),year_amt(取上個月DB值)
								((DataObject)data_thisMonth.get(i)).setValue("year_amt",((DataObject)data_lastMonth.get(j)).getValue("year_amt"));
							}
						}
						//若符合條件(上表標記V者)卻無上個月資料者，則設定值為0
						if(!changeData){
							((DataObject)data_thisMonth.get(i)).setValue("year_amt","0");
						}
					}
				}
			}
			dbData = data_thisMonth;
		}
		return dbData;
	}
	...
	private List getData_A01_A05(String S_YEAR, String S_MONTH, String bank_code, String bank_type, String acc_div) {
		...
		if (bank_type.equals("6") || (bank_type.equals("7") && !acc_div.equals("01") && !acc_div.equals("02")
				&& !acc_div.equals("04") && !acc_div.equals("05") && !acc_div.equals("07") && !acc_div.equals("08")
				&& !acc_div.equals("99") && !acc_div.equals("12") && !acc_div.equals("13"))) {
			...
			} else if (acc_div.equals("04"))
				Report_NO = "A02";
			else if (acc_div.equals("05"))
				Report_NO = "A03";
			else if (acc_div.equals("06"))
				Report_NO = "A04";
			else if (acc_div.equals("07"))
				Report_NO = "A05";
			else if (acc_div.equals("08"))
				Report_NO = "A06";//95.04.10 add by 2295
			else if (acc_div.equals("15"))
				Report_NO = "A15";//109.07.08 add by 6493
			else if (acc_div.equals("99"))
				Report_NO = "A99";//95.05.18 add by 2295
			...
			if (Report_NO.equals("A05")) {
				...
				sqlCmd.append(
						"  FROM ncacno LEFT JOIN " + Report_NO + " a01_a05 on A01_A05.acc_code = ncacno.acc_code ");
				...
				sqlCmd.append(
						" ON A01_A05.acc_code = ncacno.acc_code and A01_A05.m_year=? and A01_A05.m_month=? and A01_A05.bank_code=?");
			...
			} else if (Report_NO.equals("A15")) {//109.07.14 add A15
				String t1 = ncacno;
				...
				sqlCmd.append(" select * from " + Report_NO + " A01_A05 ");
				sqlCmd.append(" LEFT JOIN " + t1 + " ON A01_A05.acc_code = " + t1 + ".acc_code ");
				sqlCmd.append(" LEFT JOIN wlx01_limit_item on substr(" + t1 + ".acc_code,2,3) = limit_type ");
				sqlCmd.append(" where A01_A05.m_year=? and A01_A05.m_month=? and A01_A05.bank_code=? ");
				sqlCmd.append(" and " + t1 + ".acc_div=? order by " + t1 + ".acc_range");
				paramList.add(S_YEAR);
				paramList.add(S_MONTH);
				paramList.add(bank_code);
				paramList.add(acc_div);
				dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
						"m_year,m_month,amt,amt_3month,amt_6month,amt_1year,amt_2year,amt_over2year,amt_total,month_amt,year_amt");
			} else {
				sqlCmd.append(" select * from " + Report_NO + " A01_A05  LEFT JOIN " + ncacno
						+ " ON A01_A05.acc_code = " + ncacno + ".acc_code ");
				sqlCmd.append(" where A01_A05.m_year=? and A01_A05.m_month=? and A01_A05.bank_code=? ");
			...
			if (acc_div.equals("01") || acc_div.equals("02"))
				...
				Report_NO = "A02";//94.05.27 add加上A02區分農漁會
			else if (acc_div.equals("05"))
				Report_NO = "A03";
			else if (acc_div.equals("07"))
				Report_NO = "A05";//95.05.11 add加上A05區分農漁會
			else if (acc_div.equals("08"))
				Report_NO = "A06";
			else if (acc_div.equals("99"))
				Report_NO = "A99";//95.05.18 add加上A99區分農漁會
			if (acc_div.equals("12") || acc_div.equals("13")) {//104.10.12 add
				Report_NO = "A13";
			}
			if (Report_NO.equals("A05")) {//95.05.11 add A05漁會
				...
				sqlCmd.append("SELECT * ");
				sqlCmd.append(
						"  FROM ncacno_7 LEFT JOIN " + Report_NO + " a01_a05 on A01_A05.acc_code = ncacno_7.acc_code ");
				sqlCmd.append("  				 AND (A01_A05.m_year=? OR A01_A05.m_year IS NULL) ");
				sqlCmd.append("   				 AND (A01_A05.m_month=? OR A01_A05.m_month IS NULL) ");
				sqlCmd.append("   				 AND (A01_A05.bank_code=? OR A01_A05.bank_code IS NULL) ");
				...
				paramList.add(acc_div);
				dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
						"m_year,m_month,amt,amt_3month,amt_6month,amt_1year,amt_2year,amt_over2year,amt_total");
			}
		}
	...
	private List getData_A08_A12(String S_YEAR, String S_MONTH, String bank_code, String report_no) {
		...
		if (report_no.equals("A08")) {
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,warnaccount_cnt,limitaccount_cnt,erroraccount_cnt,otheraccount_cnt,depositaccount_tcnt");
		} else if (report_no.equals("A09")) {
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,over_cnt,over_amt,push_over_amt,totalamt,push_totalamt,over_total_rate");
		} else if (report_no.equals("A10")) {
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,loan1_amt,loan2_amt,loan3_amt,loan4_amt,invest1_amt,invest2_amt,invest3_amt,invest4_amt,other1_amt,other2_amt,other3_amt,other4_amt"
							+ ",loan1_baddebt,loan2_baddebt,loan3_baddebt,loan4_baddebt,build1_baddebt,build2_baddebt,build3_baddebt,build4_baddebt"
			...
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,bank_code,baddebt_amt,loss_amt,profit_amt");
		}
		return dbData;
	}
	...
	private List getData_B01(String S_YEAR, String S_MONTH, String bank_code) {
		...
		dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
				"m_year,m_month,fund_master_no,fund_sub_no,fund_next_no,budget_amt,credit_pay_amt,credit_pay_rate,remark");
	...
	private List getData_B02(String S_YEAR, String S_MONTH, String bank_code) {
		...
		paramList.add(S_MONTH);
		dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
				"m_year,m_month,run_master_no,run_sub_no,run_next_no,loan_cnt_year,loan_amt_year,loan_cnt_totacc,loan_amt_totacc,loan_cnt_bal,loan_amt_bal_subtot,loan_amt_bal_fund,loan_amt_bal_bank");
	...
	private List getData_M01(String S_YEAR, String S_MONTH, String bank_code) {
		...
		sqlCmd.append(
				"        	c.guarantee_cnt,c.loan_amt,c.guarantee_amt,c.loan_bal,c.guarantee_bal,c.over_notpush_cnt, ");
		...
		paramList.add(S_MONTH);
		dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
				"guarantee_cnt,loan_amt,guarantee_amt,loan_bal,guarantee_bal,over_notpush_cnt,over_notpush_bal,over_okpush_cnt,over_okpush_bal,repay_tot_cnt,repay_tot_amt,repay_bal_cnt,repay_bal_amt");
		return dbData;
	}
	...
	private List getData_M02(String S_YEAR, String S_MONTH, String bank_code) {
		...
		paramList.add(S_MONTH);
		dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
				"m_year,m_month,guarantee_cnt,loan_amt,guarantee_amt,loan_bal,guarantee_bal,over_notpush_cnt,over_notpush_bal,over_okpush_cnt,over_okpush_bal,repay_tot_cnt,repay_tot_amt,repay_bal_cnt,repay_bal_amt");
	...
	private List getData_M03(String S_YEAR, String S_MONTH, String bank_code, String data_range_type) {
		...
		if (data_range_type.equals("C")) { // M03的資料
			...
			paramList.add(S_MONTH);
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,guarantee_cnt_month,loan_amt_month,guarantee_amt_month,guarantee_cnt_year,loan_amt_year,guarantee_amt_year,guarantee_bal_totacc,guarantee_bal_totacc_over,repay_bal_totacc");
	...
	private List getData_M04(String S_YEAR, String S_MONTH, String bank_code) {
		...
		sqlCmd.append(
				"        loan_amt_month,loan_amt_month_p,guarantee_amt_month,guarantee_amt_month_p,guarantee_no_year, ");
	...
	private List getData_M05(String S_YEAR, String S_MONTH, String bank_code, String data_range_type) {
		...
		if (data_range_type.equals("C")) { // M05的資料
			...
			paramList.add(S_MONTH);
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,repay_cnt,repay_amt,run_notgood_cnt,run_notgood_amt,turn_out_cnt,turn_out_amt,diease_cnt,dieaserepay_amt,disaster_cnt,disaster_amt,corun_out_cnt,corun_out_amt,other_cnt,other_amt");
			...
			paramList.add(S_MONTH);
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,guarantee_no_totacc,guarantee_amt_totacc");
		}
		return dbData;
	}
	...
	private List getData_M06_M07(String S_YEAR, String S_MONTH, String bank_code, String Report_no) {
		...
		// 取得M06(M07)的資料
		sqlCmd.append(" select * from " + Report_no + ",m00_area where " + Report_no
				+ ".area_no=m00_area.area_no and m_year=? and m_month=?");
		sqlCmd.append(" and m00_area.area_no <> '1' order by input_order ");
		paramList.add(S_YEAR);
		paramList.add(S_MONTH);
		dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
				"m_year,m_month,guarantee_no_month,guarantee_amt_month,loan_amt_month,guarantee_no_year,guarantee_amt_year,loan_amt_year,guarantee_no_totacc,guarantee_amt_totacc,loan_amt_totacc,guarantee_bal_no,guarantee_bal_amt,guarantee_bal_p,loan_bal");
	...
	private List getData_M08(String S_YEAR, String S_MONTH, String bank_code, String Report_no) {
		...
		// 取得M08的資料
		sqlCmd.append(
				" select 	c.id_no,a.id_name,c.data_range,b.data_range_name,c.guarantee_no_month,c.loan_amt_month,");
		...
		paramList.add(S_MONTH);
		dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
				"m_year,m_month,guarantee_no_month,loan_amt_month,guarantee_amt_month,guarantee_bal_month,guarantee_bal_p");
	...
	private List getData_B03(String S_YEAR, String S_MONTH, String bank_code, String Report_no, int form_no) {
		...
		if (form_no == 1) { // B03_1的資料
			...
			sqlCmd.append(
					" select * from " + Report_no + "_1 a,b00_funs_item b where a.funs_master_no=b.funs_master_no ");
			...
			paramList.add(S_MONTH);
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,loan_cnt_totacc,loan_amt_totacc_fund,loan_amt_totacc_bank,loan_amt_totacc_tot,loan_cnt_bal,loan_amt_bal_fund,loan_amt_bal_bank,loan_amt_bal_tot");
		} else if (form_no == 2) { // B03_2的資料
			System.out.println("form_no=2");
			sqlCmd.append(
					" select * from " + Report_no + "_2 a,b00_funs_item b where a.funs_master_no=b.funs_master_no ");
			sqlCmd.append(" and   a.funs_sub_no=b.funs_sub_no and   a.funs_next_no=b.funs_next_no ");
			sqlCmd.append(" and   m_year=? and   m_month=? order by input_order");
			paramList.add(S_YEAR);
			paramList.add(S_MONTH);
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,loan_amt_bal,loan_amt_over,loan_rate_over");
		} else if (form_no == 3) { // B03_3的資料
			System.out.println("form_no=3");
			sqlCmd.append(
					" select * from " + Report_no + "_3 a,b00_funo_item b where a.funo_master_no=b.funo_master_no ");
			sqlCmd.append(" and   a.funo_sub_no=b.funo_sub_no and   a.funo_next_no=b.funo_next_no ");
			sqlCmd.append(" and   m_year=? and   m_month=? order by input_order");
			paramList.add(S_YEAR);
			paramList.add(S_MONTH);
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,funo_master_no,funo_sub_no,funo_next_no,funo_amt,funo_rate");
			...
			paramList.add(S_MONTH);
			dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList,
					"m_year,m_month,machine_cnt,machine_amt,land_cnt,land_amt,house_cnt,house_amt,build_cnt,build_amt,tot_cnt,tot_amt");
		}
	...
	private boolean CheckFileLock(String S_YEAR, String S_MONTH, String bank_code, String Report_no) {//檢核此檔在WML01_LOCK/WML_LOCK中有無被Lock
		...
		List paramList = new ArrayList();
		sqlCmd.append(
				"select lock_status from WML01_LOCK where m_year=? and m_month=? and bank_code=? and report_no=?");
		paramList.add(S_YEAR);
		paramList.add(S_MONTH);
		paramList.add(bank_code);
		paramList.add(Report_no);
		...
		if (WML01_LOCK.size() > 0) {
			if ((((String) ((DataObject) WML01_LOCK.get(0)).getValue("lock_status")) != null)
					&& (((String) ((DataObject) WML01_LOCK.get(0)).getValue("lock_status")).equals("Y"))) {
				System.out.println("WML01_lock true");
				lock = true;
			}
		}
		if (WML01.size() > 0) {
			if ((((String) ((DataObject) WML01.get(0)).getValue("lock_status")) != null)
					&& ((String) ((DataObject) WML01.get(0)).getValue("lock_status")).equals("Y")) {
	...
	//109.08.19 add 取得WML01最初申報年月 by 6493
	private String[] getWLX01_FIRST_APPLY(String Report_no, String bank_code) {
		StringBuffer sqlCmd = new StringBuffer();
		List paramList = new ArrayList();
		String yearMonth[] = new String[2];
		sqlCmd.append("select m_year,m_month from WML01 where REPORT_NO = ? and bank_code = ? order by m_year, m_month");
		paramList.add(Report_no);
		paramList.add(bank_code);
		List dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(), paramList, "m_year,m_month");
		System.out.println("Report_no=" + Report_no);
		System.out.println("bank_code=" + bank_code);
		System.out.println("WLX01_FIRST_APPLY.size()=" + dbData.size());
		if (dbData != null && dbData.size() != 0) {
			yearMonth[0] = (((DataObject) dbData.get(0)).getValue("m_year")).toString();
			yearMonth[1] = (((DataObject) dbData.get(0)).getValue("m_month")).toString();
		}
		return yearMonth;
	}
	...
	private List getLastMonthData(String S_YEAR, String S_MONTH, String bank_code, String Report_no, String bank_type) {
		StringBuffer sqlCmd = new StringBuffer();
		List paramList = new ArrayList();
		System.out.println("test1.S_YEAR=" + S_YEAR);
		System.out.println("test1.S_MONTH=" + S_MONTH);
		if (Integer.parseInt(S_MONTH) == 1) {//若本月為1月份是..則是抓上個年度的12月份
		...
		if (Report_no.equals("F01")) {
			...
			dbData = getData_A01_A05(S_YEAR, S_MONTH, bank_code, bank_type, "15");
		}
		System.out.println("LastMonthData.size()=" + dbData.size());
		return dbData;
	}
	
	//新增資料時，若有動態新增項目，但上個月資料卻沒有動態新增項目，則取得最新項目並將上個月資料填入
	private List getLastMonthDataAndNewItem(String S_YEAR, String S_MONTH, String bank_code, String Report_no, String bank_type) {
		String ncacno = "ncacno";
		String ncacno_7 = "ncacno_7";
		List paramList = new ArrayList();
		List data_div01 = new ArrayList();
		
		if(Report_no.equals("A15")){
			//取得上個月項目資料
			List lastMonthDataList = getLastMonthData(S_YEAR, S_MONTH, bank_code, Report_no, bank_type);
			System.out.println("lastMonthDataList.size()=" + lastMonthDataList.size());

			//取得最新項目
			List newItemDataList = null;
			String acc_div = "15";
			...
			paramList.add(Report_no);
			paramList.add(acc_div);
			newItemDataList = DBManager.QueryDB_SQLParam(sql.toString(),paramList,"update_date");
			System.out.println("newItemDataList.size()=" + newItemDataList.size());
			
			//若有上個月資料則填入，若無則最新項目
			DataObject newItemData, lastMonthData;
			String newItem_accCode, lastMonth_accCode;
			boolean dataExist = false;
			for(int i = 0; i < newItemDataList.size(); i++){
				dataExist = false;
				newItemData = (DataObject)newItemDataList.get(i);
				newItem_accCode = ((String)newItemData.getValue("acc_code")).trim();
				for(int j = 0; j < lastMonthDataList.size(); j++){
					lastMonthData = (DataObject)lastMonthDataList.get(j);
					lastMonth_accCode = ((String)lastMonthData.getValue("acc_code")).trim();
					if( newItem_accCode.equals( lastMonth_accCode ) ){
						dataExist = true;
						data_div01.add(lastMonthData);
					}
				}
				if(!dataExist){
					data_div01.add(newItemData);
				}
			}
		}
