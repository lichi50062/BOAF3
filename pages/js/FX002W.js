...
function checkWLX02Data(form) 
{
    ...
    //2018-05-30 Ethan.Shih : 檢查最近地方主管機關備查信用部分部主任函日期
    if((trimString(form.REFER_DATE_Y.value) != "" ) 
    || (trimString(form.REFER_DATE_M.value) != "" )
    || (trimString(form.REFER_DATE_D.value) != "" ))
    {				   	    
    	if (trimString(form.REFER_DATE_Y.value)  != "" ){        
    		 if(isNaN(Math.abs(form.REFER_DATE_Y.value))){
    	     	alert("最近地方主管機關備查信用部分部主任函日期(年)不可輸入文字");    
    			form.REFER_DATE_Y.focus();            
    	        return false;
    	     }
    	}else{
    		alert("最近地方主管機關備查信用部分部主任函日期(年)不可空白");
    		form.REFER_DATE_Y.focus();
    				return false;   
    	}   
    	if (trimString(form.REFER_DATE_M.value) == "" ){
    		alert("最近地方主管機關備查信用部分部主任函日期(月)不可空白");
    		form.REFER_DATE_M.focus();
    		return false;
    	}			
    	if (trimString(form.REFER_DATE_D.value) == "" ){
    		alert("最近地方主管機關備查信用部分部主任函日期(日)不可空白");
    		form.REFER_DATE_D.focus();		
    		return false;
    	}	    

    	ckDate = '' + (parseInt(form.REFER_DATE_Y.value)+1911) + '/' + form.REFER_DATE_M.value + '/' + form.REFER_DATE_D.value;
    	       
    	if( fnValidDate(ckDate) != true){
    		alert('最近地方主管機關備查信用部分部主任函日期為無效日期!!');
    	    form.REFER_DATE_D.focus();
    	    return false;
    	}    
    	form.REFER_DATE.value = ckDate;   	    	
    }
