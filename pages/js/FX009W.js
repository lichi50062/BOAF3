...
//106.04.05 fix 取消申報年季檢核 by 2295
...
function printSubmit(form,cnd,banktype,now_bank_no,muser_id){
	...
	alert('now_bank_no='+now_bank_no);
	alert('muser_id='+muser_id);
	alert('S_YEAR='+a);
	alert('S_QUARTER'+b);
	alert('form.prtStyle.value='+form.prtStyle.value);	
	*/
	if(muser_id=='A111111111' || banktype=='B' || banktype=='2'){
		form.action="/pages/FX009W_Print_B.jsp?act=new&S_YEAR="+a+"&S_MONTH="+b+"&bank_no="+now_bank_no;	 	    
		form.submit();	
		return;	
	}
	
	if( banktype=='6' || banktype=='7')
	{				
...
function printSubmitB(form,cnd,banktype,now_bank_no){
		...
		if(banktype=='B'){
			form.action="/pages/FX008W_Print_B.jsp?act=new&S_YEAR="+a+"&S_MONTH="+b+"&printStyle="+form.prtStyle.value;		    
			form.submit();
		}	
		if(banktype=='2'||banktype=='6'){ 
			form.action="/pages/FX008W_Print_2.jsp?act=new&S_YEAR="+a+"&S_MONTH="+b+"&bank_no="+now_bank_no+"&printStyle="+form.prtStyle.value;		    
			form.submit();
		}	
