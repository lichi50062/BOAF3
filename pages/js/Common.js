...
//111.01.06 fix AskInsert/AskUpdate/AskDelete,調整使用confirm() by 2295
//111.03.18 add 取得瀏覽器版本getBroswer() by 2295
...
function AskDelete() {
  ...
  if(confirm("確定要刪除嗎？"))
  //var myConfirmResult = myConfirm("Microsoft Internet Explorer","確定要刪除嗎？",2,15,8)//111.01.06 fix
  //if(myConfirmResult)  //111.01.06 fix
    return true;
  else
    return false;
}
...
function AskInsert() {
    ...
    return false;
	//return(swalconfirm('新增'));
  //if(swalconfirm('新增')){
  //var myConfirmResult = myConfirm("Microsoft Internet Explorer","確定要新增嗎？",2,15,8)//111.01.06 fix
  //if(myConfirmResult)//111.01.06 fix
  //alert('test1');
   //return true;  
  //}else{
...
function CheckQueryDate2(checkyear, checkmonth, checkdate, szWarning) {
    //alert('CheckQueryDate2='+checkyear.value);
    if(!CheckYear(checkyear))
    	return false;
    if(trimString(checkyear.value)!="" && checkmonth.options[checkmonth.selectedIndex].value!="") {
