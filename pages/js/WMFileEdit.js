...
//107.04.02 add A02.990421/990422/990621/990622增加農委會核准日期 by 2295
//107.05.02 add A02.990421/990422/990621/990622增加檢核有勾選時,日期及文號不可為空白,且日期不可大於申報年月 by 2295
//107.09.06 fix A02.990421/990422/990621/990622檢核日期不可大於申報年月時,原parseInt()在轉換08時,會轉成0,調整組合年月後.再轉成數字比較 by 2295
//108.01.03 add A05.910500增加減910109/910401/910402/910403/910404;移除910107統一農貸公積
...
//110.03.30 add A02.新增990421、990422、990622、990623、990624檢核農委會核准日期及函號是否已被撤銷，若已被撤銷則顯示該核准文號及日期已被撤銷。 by 6493
//					新增990625擔保品徵提範圍，依據農委會93年2月27日農授金字第0935000045號函規定，應於「授信政策、徵信及授信處理流程與作業手冊」訂明，經理事會決議修正，並經地方主管機關核定及理事會決議日期、地方主管機關核定日期，(1)跟(2)的日期為必要輸入欄位 by 6493
//					新增990626非會員擔保授信擔保品徵提範圍 by 6493
//110.08.13 add checkShowInsert_A99 992451/992453/992455/992461/992463/992465不檢查數字 by 2295[111.06.13取消]
...
//108.01.03 add A05.910500增加減910109/910401/910402/910403/910404;移除910107統一農貸公積
//110.03.09 add A05.920901/920902,920601風險權數原為0.5改為0.45
function addText(form){	          
	...
	if(form.assumed_amt[6].value != '' && !isNaN(changeVal(form.assumed_amt[6]))){//920901
	   a = a + parseFloat(eval(changeVal(form.assumed_amt[6])));
	   //alert('920901='+a);
	   //alert('a+920901='+a);
	}
	if(form.assumed_amt[7].value != '' && !isNaN(changeVal(form.assumed_amt[7]))){//920902
	   a = a + parseFloat(eval(changeVal(form.assumed_amt[7])));
	   //alert('920902='+a);
	   //alert('a+920902='+a);
	}
	
	if(form.assumed_amt[8].value != '' && !isNaN(changeVal(form.assumed_amt[8]))){//920710			   		
	   a = a + parseFloat(eval(changeVal(form.assumed_amt[8])));
	   //alert('920710='+a);
	   //alert('a+920710='+a);
	}
	if(form.assumed_amt[11].value != '' && !isNaN(changeVal(form.assumed_amt[11]))){//920720			  	
	   a = a + parseFloat(eval(changeVal(form.assumed_amt[11])));
	   //alert('920720='+a);
	   //alert('a+920720='+a);
	}
	if(form.assumed_amt[14].value != '' && !isNaN(changeVal(form.assumed_amt[14]))){//920730	
	   a = a + parseFloat(eval(changeVal(form.assumed_amt[14])));
	   //alert('920730='+a);
	   //alert('a+920730='+a);
	}
	if(form.assumed_amt[17].value != '' && !isNaN(changeVal(form.assumed_amt[17]))){//920740	
	   a = a + parseFloat(eval(changeVal(form.assumed_amt[17])));
	   //alert('920740='+a);
	   //alert('a+920740='+a);
	}
	if(form.assumed_amt[20].value != '' && !isNaN(changeVal(form.assumed_amt[20]))){//920750	
	   a = a + parseFloat(eval(changeVal(form.assumed_amt[20])));
	   //alert('920750='+a);
	   //alert('a+920750='+a);
	}
	
	if(form.assumed_amt[23].value != '' && !isNaN(changeVal(form.assumed_amt[23]))){//920801	
	   a = a + parseFloat(eval(changeVal(form.assumed_amt[23])));
	   //alert('920801='+a);
	   //alert('a+920801='+a);
	}
	if(form.amt[37].value != '' && !isNaN(changeVal(form.amt[37]))){//910203(2_1)特定損失所提列之備抵呆帳、損失準備及營業準備		
	   //alert('910203='+form.amt[37].value);
	   a = a - parseFloat(eval(changeVal(form.amt[37])));	  
	   //alert('a-910203='+a);
	}	
	//108.01.03 add 
	/*108.01.28 fix 移除扣除910109/910401/910402/910403/910404
	if(form.amt[30].value != '' && !isNaN(changeVal(form.amt[30]))){//910109減：備抵呆帳、損失準備及營業準備提列不足之金額 
	   //alert('910109='+form.amt[30].value);
	   a = a - parseFloat(eval(changeVal(form.amt[30])));	  
	   //alert('a-910109='+a);
	}	
	
	if(form.amt[39].value != '' && !isNaN(changeVal(form.amt[39]))){//910401全國農業金庫股票(D)  
	   //alert('910401='+form.amt[39].value);
	   a = a - parseFloat(eval(changeVal(form.amt[39])));	  
	   //alert('a-910401='+a);
	}	
	
	if(form.amt[40].value != '' && !isNaN(changeVal(form.amt[40]))){//910402財金資訊股份有限公司股票(E)
	   //alert('910402='+form.amt[40].value);
	   a = a - parseFloat(eval(changeVal(form.amt[40])));	  
	   //alert('a-910402='+a);
	}	
	
	if(form.amt[41].value != '' && !isNaN(changeVal(form.amt[41]))){//910403合作金庫銀行股票(F) 
	   //alert('910403='+form.amt[41].value);
	   a = a - parseFloat(eval(changeVal(form.amt[41])));	  
	   //alert('a-910403='+a);
	}	
	
	if(form.amt[42].value != '' && !isNaN(changeVal(form.amt[42]))){//910404上列以外長期投資(F1)
	   //alert('910404='+form.amt[42].value);
	   a = a - parseFloat(eval(changeVal(form.amt[42])));	  
	   //alert('a-910404='+a);
	}	
	*/
	//alert('920101='+form.amt[0].value+':920201='+form.amt[1].value+':920301='+form.amt[2].value+':920401='+form.amt[3].value+':920501='+form.amt[4].value+':920601='+form.amt[5].value);	
	//alert('920710='+form.amt[6].value+':920720='+form.amt[9].value+':920730='+form.amt[12].value+':920740='+form.amt[15].value+':920750='+form.amt[18].value+':920801='+form.amt[21].value);	
	//alert('910203='+form.amt[35].value+':910109='+form.amt[30].value+':910401='+form.amt[39].value+':910402='+form.amt[40].value+':910403='+form.amt[41].value+':910404='+form.amt[42].value);
	
	//alert('a='+a);	
	//alert('a.round='+Math.round(a));
	//97.03.14 add 910204改變910400.910500.91060p位置
	//108.01.03移除910107改變910400.910500.91060p位置
	form.amt[46].value= Math.round(a) ;	//95.06.01 fix 取到整數.四拾五入910500	
	form.amt[46].value=changeStr(form.amt[46]);
	//alert('form.amt[42].value='+form.amt[42].value);
    
    //94.03.17 add 91060P=910400/910500取到小數第2位(第三位四捨五入) by 2295
    var tmp = 0;        
    
    if((form.amt[45].value != '' && !isNaN(changeVal(form.amt[45]))) && (form.amt[46].value != '' && form.amt[46].value != '0')){//910500	    
    	//alert('count 91060P');    	    	
        tmp = parseFloat(eval(changeVal(form.amt[45]))) / parseFloat(eval(changeVal(form.amt[46])));    
        //alert(tmp);
        tmp = Math.round(tmp * 100000);    
        tmp = tmp / 1000;
        form.amt[47].value=tmp    
    }	
}
...
function doSubmit(form,fun,Report_no,S_YEAR,S_MONTH,bank_type) {	
	...
	if(fun == 'Insert'){		
		if(Report_no == 'A01' && (checkShowInsert_A01(form, fun))){/* modify block by Winnin.chu 2004.11.24 ver1.0*/
	   	...
	   	}else if(Report_no == 'A15' && (checkShowInsert_A15(form, fun))){/* Add by 6493 2020.07.10 ver1.0*/
	   		if(AskInsert(form)){	
	       		form.action="/pages/WMFileEdit.jsp?act="+form.act.value+"&Report_no="+Report_no+"&bank_type="+bank_type+"&test=nothing";		
	       		form.submit();
	   		}	
	...
	if(fun == 'Update') {		
	    ...
	    }else if( Report_no == 'A15' && (checkShowInsert_A15(form, fun))){/* Add block by 6493 2020.07.10 ver1.0*/
	   		if(AskUpdate(form)){	
	       		form.action="/pages/WMFileEdit.jsp?act="+form.act.value+"&Report_no="+Report_no+"&S_YEAR="+S_YEAR+"&S_MONTH="+S_MONTH+"&bank_type="+bank_type+"&test=nothing";		
	       		form.submit();
	       	}    
...
//107.04.02 add 990421/990422/990621/990622增加農委會核准日期 by 2295
//107.05.02 add 990421/990422/990621/990622增加檢核有勾選時,日期及文號不可為空白,且日期不可大於申報年月 by 2295
//110.03.30 add A02.新增990421、990422、990622、990623、990624檢核農委會核准日期及函號是否已被撤銷，若已被撤銷則顯示該核准文號及日期已被撤銷。 by 6493
//					新增990625擔保品徵提範圍，依據農委會93年2月27日農授金字第0935000045號函規定，應於「授信政策、徵信及授信處理流程與作業手冊」訂明，經理事會決議修正，並經地方主管機關核定及理事會決議日期、地方主管機關核定日期，(1)跟(2)的日期為必要輸入欄位 by 6493
//					新增990626非會員擔保授信擔保品徵提範圍 by 6493
//111.08.24 add 991311/991321增加農委會核准日期 by 2295
function checkShowInsert_A02(form, fun) {
	...
	var amt990422=0;
	var amt990622=0;
	var amt991311=0;//111.08.24 add
	var amt991321=0;//111.08.24 add
	...
	var revokeListString = form.revokeListString.value;
	var revokeList = revokeListString.split('|');
	var alertRevoke = '';
	...
	for (var i = 0; i < form.amt.length; i++) {
		...
		if(form.acc_code[i].value=='990421'){
	    	if(form.chk990421.checked == true){ 
	    		form.amt[i].value = "1";
	    		amt990421 = 1;
	    		//107.04.02 增加申報農委會核准日期
	    		//107.05.02 add 勾選990421時,日期及文號不可為空白,且日期不可大於申報年月
	    	    if(form.yy990421.value == '' || form.mm990421.value == '' || form.dd990421.value == '' || form.doc990421.value == ''){
	    	       alert('已勾選990421符合逾放比率低於1%、資本適足率高於10%且備抵呆帳覆蓋率高於100%，已申請經主管機關同意得不超過200%,農委會核准日期及文號不可為空白!!');
	    	       return false;
	    	    }	
	    	    //107.09.06 fix 原parseInt()在轉換08時,會轉成0,調整組合年月後.再轉成數字比較 by 2295
	    		if(parseInt(form.yy990421.value + form.mm990421.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){	    				
					alert('已勾選990421農委會核准日期不可大於申報年月');
					return false;    	    	    	    
	    	    }

	    		if(form.yy990421.value != '' && form.mm990421.value != '' && form.dd990421.value != '' && form.doc990421.value != ''){
	   			   form.txt990421.value = form.yy990421.value+'年'+form.mm990421.value+'月'+form.dd990421.value+'日'+'農授金字第'+form.doc990421.value+'號函';	    		
//	   			   alert(form.txt990421.value);
				}
	    	}else{
	    		form.amt[i].value = "0";
	    		amt990421 = 0;
		...
		if(form.acc_code[i].value=='990621'){
	    	if(form.chk990621.checked == true){ 
	    		...
	    		amt990621 = 1;
	    		//107.04.02 增加申報農委會核准日期
	    		//107.05.02 add 勾選990621時,日期及文號不可為空白,且日期不可大於申報年月
	    		if(form.yy990621.value == '' || form.mm990621.value == '' || form.dd990621.value == '' || form.doc990621.value == ''){
	    		   alert('已勾選990621符合逾放比率低於2%且資本適足率高於8%，已申請經主管機關同意得不超過150%,農委會核准日期及文號不可為空白!!');
	    		   return false;
	    		}	
	    		//107.09.06 fix 原parseInt()在轉換08時,會轉成0,調整組合年月後.再轉成數字比較 by 2295
	    		if(parseInt(form.yy990621.value + form.mm990621.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){	    		    		
					alert('已勾選990621農委會核准日期不可大於申報年月');
					return false;    	    	    	    
	    	    }
	    		if(form.yy990621.value != '' && form.mm990621.value != '' && form.dd990621.value != '' && form.doc990621.value != ''){
	   			   form.txt990621.value = form.yy990621.value+'年'+form.mm990621.value+'月'+form.dd990621.value+'日'+'農授金字第'+form.doc990621.value+'號函';
	   			   //alert(form.txt990621.value);
				}	
	    	}else{
	    		form.amt[i].value = "0";
	    		amt990621 = 0;
		...
		if(form.acc_code[i].value=='990422'){
	    	if(form.chk990422.checked == true){ 
	    		...
	    		amt990422 = 1;
	    		//107.04.02 增加申報農委會核准日期
	    		//107.05.02 add 勾選990422時,日期及文號不可為空白,且日期不可大於申報年月
	    		if(form.yy990422.value == '' || form.mm990422.value == '' || form.dd990422.value == '' || form.doc990422.value == ''){
	    		   alert('已勾選990422符合逾放比率低於1%、放款覆蓋率高於2%、資本適足率高於10%，已申請經主管機關同意得不受限制,農委會核准日期及文號不可為空白!!');
	    		   return false;	
	    		}	
	    		//107.09.06 fix 原parseInt()在轉換08時,會轉成0,調整組合年月後.再轉成數字比較 by 2295
	    		if(parseInt(form.yy990422.value + form.mm990422.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){	    		
					alert('已勾選990422農委會核准日期不可大於申報年月');
					return false;    	    	    	    
	    	    }
	    		if(form.yy990422.value != '' && form.mm990422.value != '' && form.dd990422.value != '' && form.doc990422.value != ''){
	   			   form.txt990422.value = form.yy990422.value+'年'+form.mm990422.value+'月'+form.dd990422.value+'日'+'農授金字第'+form.doc990422.value+'號函';
	   			   //alert(form.txt990422.value);
				}
	    		
	    		var chkUpperLimit990422 = $("input:checkbox[name='chkUpperLimit990422']:checked");
	    		if( 1 == chkUpperLimit990422.length ){
	    			if('1' == chkUpperLimit990422.val()){	    				
	    				form.amtType990422.value = '1';
	    				form.amtText990422.value = form.percent990422.value;
	    			} else {
	    				form.amtType990422.value = '2';
	    			}
	    		} else {
	    			alert('已勾選990422，須勾選一項核定上限為農委會核准之上限比率或為不受限制');
					return false; 
	    		}
	    	}else{
	    		form.amt[i].value = "0";
	    		amt990422 = 0;
	    		form.amtType990422.value = '';
	    	}
		}
		//110.03.30 add 
		if(form.acc_code[i].value=='990622'){			
	    	if(form.chk990622.checked == true){ 
	    		...
	    		amt990622 = 1;
	    		//107.04.02 增加申報農委會核准日期
	    		//107.05.02 add 勾選990622時,日期及文號不可為空白,且日期不可大於申報年月
	    		if(form.yy990622.value == '' || form.mm990622.value == '' || form.dd990622.value == '' || form.doc990622.value == ''){
	    			alert('已勾選990622符合逾放比率低於1%、放款覆蓋率高於2%、資本適足率高於10%，已申請經主管機關同意得不超過200%,農委會核准日期及文號不可為空白!!');
	    		   return false;
	    		}	    		
	    		//107.09.06 fix 原parseInt()在轉換08時,會轉成0,調整組合年月後.再轉成數字比較 by 2295
	    		if(parseInt(form.yy990622.value + form.mm990622.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){
					alert('已勾選990622農委會核准日期不可大於申報年月');					
					return false;    	    	    	    
	    	    }	
	    		if(form.yy990622.value != '' && form.mm990622.value != '' && form.dd990622.value != '' && form.doc990622.value != ''){
	   			   form.txt990622.value = form.yy990622.value+'年'+form.mm990622.value+'月'+form.dd990622.value+'日'+'農授金字第'+form.doc990622.value+'號函';
	   			   //alert(form.txt990622.value);
				}
	    	}else{
	    		form.amt[i].value = "0";
	    		amt990622 = 0;
		...
		//110.03.30 add 
		if(form.acc_code[i].value=='990623'){
	    	if(form.chk990623.checked == true){ 
	    		form.amt[i].value = "1";
	    		amt990623 = 1;
	    		if(form.yy990623.value == '' || form.mm990623.value == '' || form.dd990623.value == '' || form.doc990623.value == ''){
	    		   alert('已勾選990623符合逾放比率低於1%、放款覆蓋率高於2%、資本適足率高於10%，已申請經主管機關同意得不受限制,農委會核准日期及文號不可為空白!!');
	    		   return false;	
	    		}	
	    		if(parseInt(form.yy990623.value + form.mm990623.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){	    		
					alert('已勾選990623農委會核准日期不可大於申報年月');
					return false;    	    	    	    
	    	    }
	    		if(form.yy990623.value != '' && form.mm990623.value != '' && form.dd990623.value != '' && form.doc990623.value != ''){
	   			   form.txt990623.value = form.yy990623.value+'年'+form.mm990623.value+'月'+form.dd990623.value+'日'+'農授金字第'+form.doc990623.value+'號函';
				}
	    		
	    		var chkUpperLimit990623 = $("input:checkbox[name='chkUpperLimit990623']:checked");
	    		if( 1 == chkUpperLimit990623.length ){
	    			if('1' == chkUpperLimit990623.val()){	    				
	    				form.amtType990623.value = '1';
	    				form.amtText990623.value = form.percent990623.value;
	    			} else {
	    				form.amtType990623.value = '2';
	    			}
	    		} else {
	    			alert('已勾選990623，須勾選一項核定上限為農委會核准之上限比率或為不受限制');
					return false; 
	    		}
	    	}else{
	    		form.amt[i].value = "0";
	    		amt990623 = 0;
	    		form.amtType990623.value = '';
	    	}
		}
		//110.03.30 add 
		if(form.acc_code[i].value=='990624'){
			var chkSub990624 = $("input:checkbox[name='chkSub990624']:checked");
			var accCode = '';
			if( 1 == chkSub990624.length ){
				form.amt[i].value = chkSub990624.val();
				if('1' == chkSub990624.val()){
					if(form.yy9906241.value == '' || form.mm9906241.value == '' || form.dd9906241.value == '' || form.doc9906241.value == ''){
						alert('已勾選990624'+$('#text9906241').html()+',農委會核准日期及文號不可為空白!!');
						return false;	
					}	
					if(parseInt(form.yy9906241.value + form.mm9906241.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){	    		
						alert('已勾選990624農委會核准日期不可大於申報年月');
						return false;    	    	    	    
					}
					if(form.yy9906241.value != '' && form.mm9906241.value != '' && form.dd9906241.value != '' && form.doc9906241.value != ''){
						form.txt990624.value = form.yy9906241.value+'年'+form.mm9906241.value+'月'+form.dd9906241.value+'日'+'農授金字第'+form.doc9906241.value+'號函';
					}
				} else if ('2' == chkSub990624.val()){
					if(form.yy9906242.value == '' || form.mm9906242.value == '' || form.dd9906242.value == '' || form.doc9906242.value == ''){
						alert('已勾選990624'+$('#text9906242').html()+',農委會核准日期及文號不可為空白!!');
						return false;	
					}	
					if(parseInt(form.yy9906242.value + form.mm9906242.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){	    		
						alert('已勾選990624農委會核准日期不可大於申報年月');
						return false;    	    	    	    
					}
					if(form.yy9906242.value != '' && form.mm9906242.value != '' && form.dd9906242.value != '' && form.doc9906242.value != ''){
						form.txt990624.value = form.yy9906242.value+'年'+form.mm9906242.value+'月'+form.dd9906242.value+'日'+'農授金字第'+form.doc9906242.value+'號函';
					}
				} else {
					form.txt990624.value = '';
				}
			} else {
				alert('須勾選一項990624 非會員擔保授信擔保品種類(以下3選1) ');
				return false; 
			}
		}
		//110.03.30 add 
		if(form.acc_code[i].value=='990625'){
			form.amt[i].value = '0';
			if(form.yy9906251.value == '' || form.mm9906251.value == '' || form.dd9906251.value == ''){
				alert('990625,'+$('#text9906251').html()+',須填寫完整!!');
				return false;	
			}
			if(parseInt(form.yy9906251.value + form.mm9906251.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){	    		
				alert('990625,'+$('#text9906251').html()+'不可大於申報年月');
				return false;    	    	    	    
			}
			if(form.yy9906251.value != '' && form.mm9906251.value != '' && form.dd9906251.value != ''){
				form.amt[i].value = '1';
				form.txt9906251.value = form.yy9906251.value+'年'+form.mm9906251.value+'月'+form.dd9906251.value+'日';
			}
			
			if(form.yy9906252.value == '' || form.mm9906252.value == '' || form.dd9906252.value == ''){
				alert('990625,'+$('#text9906252').html()+',不可為空白!!');
				return false;	
			}
			if(parseInt(form.yy9906252.value + form.mm9906252.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){	    		
				alert('990625,'+$('#text9906252').html()+'不可大於申報年月');
				return false;    	    	    	    
			}
			if(form.yy9906252.value != '' && form.mm9906252.value != '' && form.dd9906252.value != ''){
				form.amt[i].value = '1';
				form.txt9906252.value = form.yy9906252.value+'年'+form.mm9906252.value+'月'+form.dd9906252.value+'日';
			}
		}
		//110.03.30 add 
		if(form.acc_code[i].value=='990626'){
			var chk990626 = $("input:checkbox[name='chk990626']:checked");
			var accCode = '';
			if( 1 == chk990626.length ){
				form.amt[i].value = chk990626.val();
				if('1' == chk990626.val()){
					if(form.HSIEN_ID_9906261.value == ''){
						alert('已勾選990626'+$('#text9906261').html()+',縣市別必填!!');
						return false;						
					}else{
						form.txt990626.value = form.HSIEN_ID_9906261.value;
						form.txt9906261.value = '';
						form.txt9906262.value = '';
					}
				} else if ('2' == chk990626.val()){
					if(form.HSIEN_ID_9906262.value == ''){
						alert('已勾選990626'+$('#text9906262').html()+' (1)前開處理流程與作業手冊所訂之毗鄰之直轄市、縣(市),縣市別必填!!');
						return false;						
					} else if(form.HSIEN_ID_AREA_ID_9906262_1.value == '' || form.HSIEN_ID_AREA_ID_9906262_2.value == ''){
						alert('已勾選990626'+$('#text9906262').html()+' (2)毗鄰二鄉、鎮、市、區,縣市鄉鎮別必填!!');
						return false;						
					} else {
						form.txt990626.value = form.HSIEN_ID_9906262.value;
						form.txt9906261.value = form.HSIEN_ID_AREA_ID_9906262_1.value;
						form.txt9906262.value = form.HSIEN_ID_AREA_ID_9906262_2.value;
					}
				} else if ('4' == chk990626.val()){
					if(form.HSIEN_ID_AREA_ID_9906264_1.value == '' || form.HSIEN_ID_AREA_ID_9906264_2.value == ''){
						alert('已勾選990626'+$('#text9906264').html()+',縣市鄉鎮別必填!!');
						return false;						
					} else {
						form.txt990626.value = '';
						form.txt9906261.value = form.HSIEN_ID_AREA_ID_9906264_1.value;
						form.txt9906262.value = form.HSIEN_ID_AREA_ID_9906264_2.value;
					}
				} else {
					form.txt990626.value = '';
					form.txt9906261.value = '';
				...
				alert('須勾選一項990626 非會員擔保授信擔保品徵提範圍  ');
				return false; 
			}
		}
		
		//111.08.24 add 
		if(form.acc_code[i].value=='991311'){
	    	if(form.chk991311.checked == true){ 
	    		form.amt[i].value = "1";
	    		amt991311 = 1;
	    		//107.04.02 增加申報農委會核准日期
	    		//107.05.02 add 勾選991311時,日期及文號不可為空白,且日期不可大於申報年月
	    	    if(form.yy991311.value == '' || form.mm991311.value == '' || form.dd991311.value == '' || form.doc991311.value == ''){
	    	       alert('已勾選991311符合逾放比率低於1%、資本適足率高於12%、放款覆蓋率高於2%，已申請經主管機關同意非會員及其同一關係人放款總額不得超過信用部前一年度決算淨值之15%,農委會核准日期及文號不可為空白!!');
	    	       return false;
	    	    }	
	    	    //107.09.06 fix 原parseInt()在轉換08時,會轉成0,調整組合年月後.再轉成數字比較 by 2295
	    		if(parseInt(form.yy991311.value + form.mm991311.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){	    				
					alert('已勾選991311農委會核准日期不可大於申報年月');
					return false;    	    	    	    
	    	    }

	    		if(form.yy991311.value != '' && form.mm991311.value != '' && form.dd991311.value != '' && form.doc991311.value != ''){
	   			   form.txt991311.value = form.yy991311.value+'年'+form.mm991311.value+'月'+form.dd991311.value+'日'+'農授金字第'+form.doc991311.value+'號函';	    		
	   			   //alert(form.txt991311.value);
				}
	    	}else{
	    		form.amt[i].value = "0";
	    		amt991311 = 0;
		...
		//111.08.24 add 
		if(form.acc_code[i].value=='991321'){
	    	if(form.chk991321.checked == true){ 
	    		form.amt[i].value = "1";
	    		amt991321 = 1;
	    		//107.04.02 增加申報農委會核准日期
	    		//107.05.02 add 勾選991321時,日期及文號不可為空白,且日期不可大於申報年月
	    	    if(form.yy991321.value == '' || form.mm991321.value == '' || form.dd991321.value == '' || form.doc991321.value == ''){
	    	       alert('已勾選991321符合逾放比率低於1%、資本適足率高於12%、放款覆蓋率高於2%，已申請經主管機關同意非會員及其同一關係人無擔保放款總額不得超過信用部前一年度決算淨值之3%,農委會核准日期及文號不可為空白!!');
	    	       return false;
	    	    }	
	    	    //107.09.06 fix 原parseInt()在轉換08時,會轉成0,調整組合年月後.再轉成數字比較 by 2295
	    		if(parseInt(form.yy991321.value + form.mm991321.value ) > parseInt(form.S_YEAR.value + form.S_MONTH.value)){	    				
					alert('已勾選991321農委會核准日期不可大於申報年月');
					return false;    	    	    	    
	    	    }

	    		if(form.yy991321.value != '' && form.mm991321.value != '' && form.dd991321.value != '' && form.doc991321.value != ''){
	   			   form.txt991321.value = form.yy991321.value+'年'+form.mm991321.value+'月'+form.dd991321.value+'日'+'農授金字第'+form.doc991321.value+'號函';	    		
	   			   //alert(form.txt991321.value);
				}
	    	}else{
	    		form.amt[i].value = "0";
	    		amt991321 = 0;
		...
		for(var j=0; j<revokeList.length; j++){
			checkRevoke = false;
			if(form.acc_code[i].value == revokeList[j].split(',')[0]){
				if('990624' == form.acc_code[i].value && revokeList[j].split(',')[1] == $("input:checkbox[name='chkSub990624']:checked").val()){
					if('1'==$("input:checkbox[name='chkSub990624']:checked").val() && form.doc9906241.value == revokeList[j].split(',')[2]){
						checkRevoke = true;
					}else if('2'==$("input:checkbox[name='chkSub990624']:checked").val() && form.doc9906242.value == revokeList[j].split(',')[2]){
						checkRevoke = true;
					}
				}else{
					if(form.amt[i].value == '1'){						
						if('990421' == form.acc_code[i].value && form.doc990421.value == revokeList[j].split(',')[2]
						|| '990422' == form.acc_code[i].value && form.doc990422.value == revokeList[j].split(',')[2]
						|| '990622' == form.acc_code[i].value && form.doc990622.value == revokeList[j].split(',')[2]
						|| '990623' == form.acc_code[i].value && form.doc990623.value == revokeList[j].split(',')[2]){
							checkRevoke = true;
						}
					}
				}
			...
			if(checkRevoke){
				if("" == alertRevoke){
					alertRevoke = form.acc_code[i].value;
				}else{
					alertRevoke += "、" + form.acc_code[i].value;
				}
			}
	...
	if(amt990421 ==1 && form.txt990421.value == ''){
       alert('若已勾選990421符合逾放比率低於1%、資本適足率高於10%且備抵呆帳覆蓋率高於100%，已申請經主管機關同意得不超過200%,則其農委會核准日期及函號必須輸入');
       return false;
    }
    if(amt990422 ==1 && form.txt990422.value == ''){
       alert('若已勾選990422符合逾放比率低於1%、放款覆蓋率高於2%、資本適足率高於10%，已申請經主管機關同意得不受限制,則其農委會核准日期及函號必須輸入');
       return false;
    }
    if(amt990621 ==1 && form.txt990621.value == ''){
       alert('若已勾選990621符合逾放比率低於2%且資本適足率高於8%，已申請經主管機關同意得不超過150%,則其農委會核准日期及函號必須輸入');
       return false;
    }	
    if(amt990622 ==1 && form.txt990622.value == ''){
       alert('若已勾選990622符合逾放比率低於1%、放款覆蓋率高於2%、資本適足率高於10%，已申請經主管機關同意得不超過200%,則其農委會核准日期及函號必須輸入');
       return false;
    }	
    //111.08.24 add
    if(amt991311 ==1 && form.txt991311.value == ''){
       alert('若已勾選991311符合逾放比率低於1%、資本適足率高於12%、放款覆蓋率高於2%，已申請經主管機關同意非會員及其同一關係人放款總額不得超過信用部前一年度決算淨值之15%,則其農委會核准日期及函號必須輸入');
       return false;
    }
    //111.08.24 add
    if(amt991321 ==1 && form.txt991321.value == ''){
       alert('若已勾選991321符合逾放比率低於1%、資本適足率高於12%、放款覆蓋率高於2%，已申請經主管機關同意非會員及其同一關係人無擔保放款總額不得超過信用部前一年度決算淨值之3%,則其農委會核准日期及函號必須輸入');
       return false;
    }
...
//100.08.04 fix A99.992100農(漁)會全體資產總額必須申報,且不可為0/負數 by 2295
//101.06.29 add 992300信用部員工人數需與總機構從事信用業務員工人數一致,才可新增 by 2295
//110.08.13 add	992451/992453/992455/992461/992463/992465不檢查數字 by 2295[111.06.13取消]		 			
function checkShowInsert_A99(form, fun) {			
	...
	for(var i=0;i<form.elements.length;i++){
		//if(i==53 || i==59 || i==65 || i==74 || i==80 || i==86) continue;//110.08.13 add	992451/992453/992455/992461/992463/992465不檢查數字[111.06.13取消]							
		if(form.elements[i].type=='text' && (!checkNumber(form.elements[i]))){
			//alert("i="+i);
			return false;
		}	
...
function checkShowInsert_A15(form, fun) {			
	if (fun == 'new') {
		if (!checkSingleYM(form.S_YEAR, form.S_MONTH))
			return false;
	}	
...
function changeStrA15Add(T1) {
	var children = document.getElementsByName(T1.parentNode.name)[1].childNodes;
	var originNode = null;
	var yearAmtNode = null;
	if(children.length != 0){
		for (var i = 0; i < children.length; i++) {
			if(children[i].name == 'origin'){
				originNode = children[i];
			}
			if(children[i].name == 'year_amt'){
				yearAmtNode = children[i];
			}
		}
	}
	if(originNode != null && yearAmtNode != null){
		var value = parseInt(changeVal(T1));
		var originValue = parseInt(changeVal(originNode)==""?"0":changeVal(originNode));
		
		if (isNaN(Math.abs(value)) && changeVal(originNode)==""){
			value = "";
		} else if (isNaN(Math.abs(value)) && changeVal(originNode)!=""){
			value = originValue
		}else{
			value = Math.abs(value);
			value = value + originValue == 0 ? "" : value + originValue;
		}

		yearAmtNode.value = value;
		yearAmtNode.value = changeStr(yearAmtNode);		
	}
	
	return Math.abs(changeStr(T1))==0?"":changeStr(T1);
}
function changeStrA15Month01(T1) {
	var children = document.getElementsByName(T1.parentNode.name)[1].childNodes;
	var yearAmtNode = null;
	if(children.length != 0){
		for (var i = 0; i < children.length; i++) {
	...
	if(yearAmtNode != null){
		...
		if (isNaN(Math.abs(value)) || value==0){
			value = "";
		}

		yearAmtNode.value = Math.abs(value);
		yearAmtNode.value = changeStr(yearAmtNode);		
	}
