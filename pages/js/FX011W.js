...
function changeOption(){
    ...
    */
    var xmlDoc = $.parseXML($("xml[id=BankNoXML]").html()) ;    
    var data = $(xmlDoc).find("data") ;
    var oOption;
    document.UpdateForm.examine.length = 0;
    $(data).each(function (i) {      	
     	if ($(this).find("banktype").text() == document.UpdateForm.tbank_no_examine.value)  {
            oOption = document.createElement("OPTION");  			
			oOption.text= $(this).find("bankname").text();
  			oOption.value=$(this).find("bankvalue").text();
  			document.UpdateForm.examine.add(oOption); 
    	}
...
if(cnd =='add'){
	...
	if(form.m_year.value == ''){
	   alert("年度必須輸入!!");
		...
		form.action="/pages/FX011W.jsp?act=Print&m2_name="+bank_no+"&printStyle="+form.prtStyle.value;		    
		form.submit();
	}
}
