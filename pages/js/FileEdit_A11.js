...
function printSubmit(form,bank_no,bank_name){
	...
	//108.05.10 add 報表格式挑選
	var prtType = new Array ("xls","ods","pdf");
	for ( i=0; i<3; i++ ) {
	...
	form.action="/pages/WMFileEdit_A11_Print.jsp?bank_name="+bank_name+"&bank_no="+bank_no+"&s_year="+form.s_year.value+"&s_month="+form.s_month.value+"&Unit="+form.Unit.value+"&printStyle="+form.prtStyle.value;	
	form.submit();
}
...
//111.03.04 增加取得瀏覽器版本,IE取得的系統年度為2022年,Edge/Chrome取得的系統年度為122年
	...
	if(cnd=="nextPg"){
		if(form.radio1[1].checked){
			...
			if(form.radio2[0].checked){
				if(document.UpdateForm.case_cnt.value==""){
					alert("請輸入項數!");
					return false;
				}else if(document.UpdateForm.case_cnt.value=="1"){
					alert("項數至少2筆以上!");
					return false;
				}
				if(!checkNumber_A11(document.UpdateForm.case_cnt)){
			    	alert("項數 須為數字!");
			   	    return false;
				}
				
			}else if(form.radio2[1].checked){
				document.UpdateForm.case_cnt.value="1";
			}
			alert("開始新增"+document.UpdateForm.s_year.value+"年"+document.UpdateForm.s_month.value+"月聯貸款案件資料!!");
		}
		form.submit();
	}
	if(cnd == "New"){
		today = new Date();
		var abc = getBroswer();//111.03.18 取得瀏覽器版本
    
		if(abc.broswer == "IE"){		
			today_year = today.getYear()-1911;
		}else{
			today_year = today.getYear()-11;
		}
		...
		//alert('s_year='+document.UpdateForm.s_year.value);
		//alert('s_month='+document.UpdateForm.s_month.value);
		if(document.UpdateForm.s_year.value!= today_year || 
		   document.UpdateForm.s_month.value != today_month ){
