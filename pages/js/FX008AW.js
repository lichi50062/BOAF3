 ...
 *111.03.04 調整xml取得方式 by 2295
 *111.03.04 增加取得瀏覽器版本,IE取得的系統年度為2022年,Edge/Chrome取得的系統年度為122年 by 2295
 */
//==========================================================
...
//====抄錄最近一期的函報資料======================================================
//111.03.4 fix 調整xml取得方式
function LoadRecentData(form,xml){
	
	var xmlDoc = $.parseXML($("xml[id=Rpt_BOAF]").html()) ;    
    ...
    $(data).each(function (i) {
    	...
    	//做抄錄的動作
		document.UpdateForm.DocNo.value = $(this).find("docno").text();
		document.UpdateForm.BOAF_DocNo.value = $(this).find("boafdocno").text();		
		document.UpdateForm.ApplyOK_y.value =  $(this).find("apoky").text();
		setSelect(document.UpdateForm.ApplyOK_m,$(this).find("apokm").text());
		setSelect(document.UpdateForm.ApplyOK_d,$(this).find("apokd").text());
		document.UpdateForm.BOAF_y.value = $(this).find("boafy").text();
		setSelect(document.UpdateForm.BOAF_m,$(this).find("boafm").text());
		setSelect(document.UpdateForm.BOAF_d,$(this).find("boafd").text());    	
    })
    ;
...
function checkCurrentDate(year,month,day,txt){
	var abc = getBroswer();//111.03.04 取得瀏覽器版本
    //alert("broswer:"+abc.broswer+" version:"+abc.version);
   
	var today = new Date();
	var nowYear;
	if(abc.broswer == "IE"){		
	 	nowYear= today.getYear()-1911; 
    }else{
    	nowYear= today.getYear()-11;
    }	
